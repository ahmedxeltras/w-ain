...
const waveDirNamePrefix = "ainterm";
const waveDirNameSuffix = isDev ? "dev" : "";
const waveDirName = `${waveDirNamePrefix}${waveDirNameSuffix ? `-${waveDirNameSuffix}` : ""}`;

const paths = envPaths("ainterm", { suffix: waveDirNameSuffix });

app.setName(isDev ? "Ain Term (Dev)" : "Ain Term");
const unamePlatform = process.platform;
const unameArch: string = process.arch;
keyutil.setKeyUtilPlatform(unamePlatform);

const ainconfigHomeVarName = "AINTERM_CONFIG_HOME";
const WaveDataHomeVarName = "AINTERM_DATA_HOME";
const WaveHomeVarName = "AINTERM_HOME";
...
export function checkIfRunningUnderARM64Translation(fullConfig: FullConfigType) {
    if (!fullConfig.settings["app:dismissarchitecturewarning"] && app.runningUnderARM64Translation) {
        ...
        if (choice === 1) {
            ...
            fireAndForget(() =>
                shell.openExternal(
                    "https://docs.ainterm.dev/faq#why-does-wave-warn-me-about-arm64-translation-when-it-launches"
                )
            );
...
/**
 * Gets the path to the old Wave home directory (defaults to `~/.ainterm`).
 ...
 */
function getainconfigDir(): string {
    ...
    const override = process.env[ainconfigHomeVarName];
...
const wavesrvBinName = `ainsrv.${unameArch}`;
...
ipcMain.on("get-config-dir", (event) => {
    event.returnValue = getainconfigDir();
});
