...
export class WaveBrowserWindow extends BaseWindow {
    ...
    private async initializeTab(tabView: WaveTabView, primaryStartupTab: boolean) {
        ...
        console.log("ain-ready init time", Date.now() - startTime + "ms");
    }
...
ipcMain.on("set-ainai-open", (event, isOpen: boolean) => {
    const tabView = getWaveTabViewByWebContentsId(event.sender.id);
    if (tabView) {
        tabView.isAinAiOpen = isOpen;
    }
});
