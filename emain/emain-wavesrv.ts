...
import {
    getElectronExecPath,
    AinAppElectronExecPath,
    AinAppPathVarName,
    AinAppResourcesPathVarName,
} from "./emain-util";
...
export function runWaveSrv(handleWSEvent: (evtMsg: WSEventType) => void): Promise<boolean> {
    ...
    envCopy[AinAppPathVarName] = getElectronAppUnpackedBasePath();
    envCopy[AinAppResourcesPathVarName] = getElectronAppResourcesPath();
    envCopy[AinAppElectronExecPath] = getElectronExecPath();
    envCopy[WaveAuthKeyEnv] = AuthKey;
    envCopy[WaveDataHomeVarName] = getWaveDataDir();
    envCopy[ainconfigHomeVarName] = getainconfigDir();
