...
function findHighestLogNumber(logsDir: string): number {
    ...
    for (const file of files) {
        const match = file.match(/^AinApp\.(\d+)\.log$/);
        if (match) {
...
function rotateLogIfNeeded(): string | null {
    ...
    const logFile = path.join(waveDataDir, "AinApp.log");
    ...
    if (stats.size > 10 * 1024 * 1024) {
        ...
        const rotatedPath = path.join(logsDir, `AinApp.${nextNum}.log`);
...
const loggerTransports: winston.transport[] = [
    new winston.transports.File({ filename: path.join(getWaveDataDir(), "AinApp.log"), level: "info" }),
];
if (isDev) {
