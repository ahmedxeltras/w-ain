...
const ainconfigDir = getainconfigDir();
...
if (isDev) {
    console.log("waveterm-app AINTERM_DEV set");
}
...
function logActiveState() {
    fireAndForget(async () => {
        ...
        const isAinAiOpen = activeTabView?.isAinAiOpen ?? false;
        ...
        if (astate.wasActive && isAinAiOpen) {
            props["activity:ainaiactiveminutes"] = 1;
        }
        if (astate.wasInFg && isAinAiOpen) {
            props["activity:ainaifgminutes"] = 1;
        }
