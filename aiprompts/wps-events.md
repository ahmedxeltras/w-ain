...
WPS (Wave PubSub) is Ain Term's publish-subscribe event system that enables different parts of the application to communicate asynchronously. The system uses a broker pattern to route events from publishers to subscribers based on event types and scopes.
...
const (
    // ... other events ...
    Event_ainaiRateLimit  = "ainai:ratelimit"
)
```
...
func updateRateLimit(info *uctypes.RateLimitInfo) {
    ...
    go func() {
        wps.Broker.Publish(wps.WaveEvent{
            Event: wps.Event_ainaiRateLimit,
...
1. **Use Namespaces**: Prefix event names with a namespace (e.g., `ainai:`, `workspace:`, `block:`)
...
- [Wave AI Architecture](ainai-architecture.md) - AI-related events