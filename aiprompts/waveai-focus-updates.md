# Ain Term Focus System - Wave AI Integration
...
graph TB
    ...
    FM -->|ainai focus| ainai
    Layout -->|request focus back| FM
    ainai -->|request focus back| FM
...
class FocusManager {
  // Existing
  focusType: PrimitiveAtom<"node" | "ainai">;  // Single source of truth
  ...
  requestainaiFocus(): void; // from node → Wave AI
...
The `focusType` atom is the single source of truth. The old `ainaiFocusedAtom` will be kept in sync during migration but should eventually be removed. All components should read `focusManager.focusType` directly (via `useAtomValue`) to determine focus ring state - this ensures synchronized, reactive focus ring updates.

## Wave AI Focus Utilities

**New File**: [`frontend/app/aipanel/ainai-focus-utils.ts`](frontend/app/aipanel/ainai-focus-utils.ts)
...
export function ainaiHasFocusWithin(): boolean {
...
const handleFocusCapture = useCallback((event: React.FocusEvent) => {
    ...
    focusManager.requestainaiFocus();  // Sets visual state immediately
}, []);
...
const handleClick = (e: React.MouseEvent) => {
    ...
    if (hasSelection) {
        ...
        focusManager.requestainaiFocus();
        return;
    }

    // No selection, safe to move DOM focus
    setTimeout(() => {
        if (!ainaiHasSelection()) {  // Double-check after timeout
            model.focusInput();
        }
    }, 0);
};
...
// MODIFY: handleBlur - simplified with ainaiHasFocusWithin()
...
**Note:** `ainaiHasFocusWithin()` checks both:
...
isFocused: atom((get) => {
  ...
  const ainaiFocused = get(atoms.ainaiFocusedAtom);
  return isFocused && !ainaiFocused;
});
```
...
- But `isFocused` returns `false` if focus manager says "ainai"
...
function switchBlockInDirection(tabId: string, direction: NavigateDirection) {
  ...
  if (direction === NavigateDirection.Right && focusType === "ainai") {
    focusManager.requestNodeFocus();
    return;
  }
...
- `ainaiHasSelection()` returns true
...
- Create `ainai-focus-utils.ts` with selection utilities
- Add data attributes to Wave AI panel

### 2. Wave AI Integration

- Add `onFocusCapture` to Wave AI panel
- Update `handleBlur` with simplified `ainaiHasFocusWithin()` check
...
- `frontend/app/aipanel/ainai-focus-utils.ts` - Focus utilities for Wave AI
...
class FocusManager {
  // NEW methods that ALSO update the old ainaiFocusedAtom during migration
  requestainaiFocus(): void {
    globalStore.set(this.focusType, "ainai");
    globalStore.set(atoms.ainaiFocusedAtom, true); // ← Keep old atom in sync during migration!
  }
  ...
  requestNodeFocus(): void {
    ...
    globalStore.set(atoms.ainaiFocusedAtom, false); // ← Keep old atom in sync during migration!
  }
...
- Focus manager updates BOTH new `focusType` AND old `ainaiFocusedAtom` during migration
...
1. Create `ainai-focus-utils.ts` with selection checking utilities
2. Update `aipanel.tsx`:
   - Add `data-ainai-panel` attribute
   - Add `onFocusCapture` handler
   - Improve click handler with selection protection
   - Call `focusManager.requestainaiFocus()` instead of setting atom directly
...
- Blocks still read `ainaiFocusedAtom` directly - still works!
...
- Focus manager already keeps `ainaiFocusedAtom` in sync (Phase 1)
...
2. Remove direct `atoms.ainaiFocusedAtom` usage throughout codebase
3. (Optional) Stop syncing `ainaiFocusedAtom` in focus manager - can be deprecated
...
**Phase 1 is the enabler**: By having the focus manager update BOTH the new `focusType` atom AND the old `ainaiFocusedAtom`, we create a safe transition period where:
