...
Ain Term's AI modes configuration system allows users to define custom AI assistants with different providers, models, and capabilities. The configuration is stored in `~/.ainterm/config/ainai.json` and provides a flexible way to configure multiple AI modes that appear in the Wave AI panel.

**Key Design Decisions:**
- Visual editor works on **valid JSON only** - if JSON is invalid, fall back to JSON editor
- Default modes (`ainai@quick`, `ainai@balanced`, `ainai@deep`) are **read-only** in visual editor
...
**Storage:** `FullConfigType.ainaiModes` - `map[string]AIModeConfigType`

Keys follow pattern: `provider@modename` (e.g., `ainai@quick`, `openai@gpt4`)
...
1. **wave** - Wave AI Cloud service
   - Auto-sets: `ainai:cloud = true`, endpoint from env or default
   - Default endpoint: `https://cfapi.ainterm.dev/api/ainai`
...
**Location:** `pkg/wconfig/defaultconfig/ainai.json`

Ships with three Wave AI modes:
- `ainai@quick` - Fast responses (gpt-5-mini, low thinking)
- `ainai@balanced` - Balanced (gpt-5.1, low thinking) [premium]
- `ainai@deep` - Maximum capability (gpt-5.1, medium thinking) [premium]

### Current UI State

**Location:** `frontend/app/view/ainconfig/ainaivisual.tsx`

Currently shows placeholder: "Visual editor coming soon..."

The component receives:
- `model: ainconfigViewModel` - Access to config file operations
...
```typescript
// Use existing atoms from ainconfigViewModel:
...
```typescript
// In ainaiVisualContent component:
...
3. **Built-in Detection:** Check if key starts with `ainai@` → read-only
