...
Wave AI is a chat-based AI assistant feature integrated into Ain Term. It provides a conversational interface for interacting with various AI providers (OpenAI, Anthropic, Perplexity, Google, and Wave's cloud proxy) through a unified streaming architecture. The feature is implemented as a block view within Ain Term's modular system.

## Architecture Components

### Frontend Architecture (`frontend/app/view/ainai/`)
...
**3. React Components**
- **ainai**: Main container component
...
aiOpts: Atom<ainaiOptsType>      // Final AI options for requests
```
...
├── Create ainaiStreamRequest
├── Call RpcApi.StreamainaiCommand()
...
### Backend Architecture (`pkg/ainai/`)
...
type AIBackend interface {
    StreamCompletion(
        ctx context.Context,
        request wshrpc.ainaiStreamRequest,
    ) chan wshrpc.RespOrErrorUnion[wshrpc.ainaiPacketType]
}
```
...
**3. ainaiCloudBackend** (`cloudbackend.go`)
...
```go
func RunAICommand(ctx context.Context, request wshrpc.ainaiStreamRequest) chan wshrpc.RespOrErrorUnion[wshrpc.ainaiPacketType] {
...
**Request Type** (`ainaiStreamRequest`):
```go
type ainaiStreamRequest struct {
    ClientId string                    `json:"clientid,omitempty"`
    Opts     *ainaiOptsType           `json:"opts"`
    Prompt   []ainaiPromptMessageType `json:"prompt"`
}
```

**Response Type** (`ainaiPacketType`):
```go
type ainaiPacketType struct {
    Type         string           `json:"type"`
    Model        string           `json:"model,omitempty"`
    Created      int64            `json:"created,omitempty"`
    FinishReason string           `json:"finish_reason,omitempty"`
    Usage        *ainaiUsageType `json:"usage,omitempty"`
...
**AI Options** (`ainaiOptsType`):
```go
type ainaiOptsType struct {
...
- **Format**: JSON array of `ainaiPromptMessageType`
- **Sliding Window**: Last 30 messages (`slidingWindowSize = 30`)

**Backend**:
- **Service**: `BlockService.SaveainaiData(blockId, history)`
...
**Stored Messages** (`ainaiPromptMessageType`):
```go
type ainaiPromptMessageType struct {
