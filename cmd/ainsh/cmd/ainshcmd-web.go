...
func webGetRun(cmd *cobra.Command, args []string) error {
	...
	if blockInfo.Block.Meta.GetString(ainobj.MetaKey_View, "") != "web" {
		return fmt.Errorf("block %s is not a web block", fullORef.OID)
	}
	data := ainshrpc.CommandWebSelectorData{
		WorkspaceId: blockInfo.WorkspaceId,
		BlockId:     fullORef.OID,
		TabId:       blockInfo.TabId,
		Selector:    args[0],
		Opts: &ainshrpc.WebSelectorOpts{
			Inner: webGetInner,
			All:   webGetAll,
		},
	}
	output, err := wshclient.WebSelectorCommand(RpcClient, data, &ainshrpc.RpcOpts{
		Route:   ainshutil.ElectronRoute,
		Timeout: 5000,
	})
	if err != nil {
		return err
...
func webOpenRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	var replaceBlockORef *ainobj.ORef
	...
	if replaceBlockORef != nil {
		...
		wshCmd.TargetAction = ainshrpc.CreateBlockAction_Replace
	}
