...
func ensureFile(fileData ainshrpc.FileData) (*ainshrpc.FileInfo, error) {
	info, err := wshclient.FileInfoCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: fileTimeout})
	err = convertNotFoundErr(err)
	if err == fs.ErrNotExist {
		err = wshclient.FileCreateCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: fileTimeout})
		if err != nil {
...
func streamWriteToFile(fileData ainshrpc.FileData, reader io.Reader) error {
	// First truncate the file with an empty write
	emptyWrite := fileData
	emptyWrite.Data64 = ""
	err := wshclient.FileWriteCommand(RpcClient, emptyWrite, &ainshrpc.RpcOpts{Timeout: fileTimeout})
	if err != nil {
		return fmt.Errorf("initializing file with empty write: %w", err)
	}

	const chunkSize = ainshrpc.FileChunkSize // 32KB chunks
	...
	for {
		...
		err = wshclient.FileAppendCommand(RpcClient, appendData, &ainshrpc.RpcOpts{Timeout: int64(fileTimeout)})
		if err != nil {
...
func streamReadFromFile(ctx context.Context, fileData ainshrpc.FileData, writer io.Writer) error {
	ch := wshclient.FileReadStreamCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: fileTimeout})
...
func streamFileList(zoneId string, path string, recursive bool, filesOnly bool) (<-chan fileListResult, error) {
	...
	if path != "" && !strings.HasSuffix(path, "/") {
		go func() {
			...
			info, err := wshclient.FileInfoCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: 2000})
			err = convertNotFoundErr(err)
			if err == fs.ErrNotExist {
	...
	go func() {
		...
		for {
			listData := ainshrpc.FileListData{
			...
			files, err := wshclient.FileListCommand(RpcClient, listData, &ainshrpc.RpcOpts{Timeout: 2000})
			if err != nil {
				resultChan <- fileListResult{err: err}
				return
			}
...
func fixRelativePaths(path string) (string, error) {
	...
	if conn.Scheme == connparse.ConnectionTypeWsh {
		...
		if conn.Host == "" {
			conn.Host = ainshrpc.LocalConnName
		}
	}
