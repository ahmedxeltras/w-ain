...
func aiRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	var allFiles []ainshrpc.AIAttachedFile
	var stdinUsed bool
	...
	for _, filePath := range args {
		...
		allFiles = append(allFiles, ainshrpc.AIAttachedFile{
			Name:   fileName,
			Type:   mimeType,
			Size:   len(data),
	...
	tabId := os.Getenv("AINTERM_TABID")
	if tabId == "" {
		return fmt.Errorf("AINTERM_TABID environment variable not set")
	}

	route := ainshutil.MakeTabRouteId(tabId)

	if aiNewBlockFlag {
		newChatData := ainshrpc.CommandWaveAIAddContextData{
			NewChat: true,
		}
		err := wshclient.AinAiAddContextCommand(RpcClient, newChatData, &ainshrpc.RpcOpts{
			Route:   route,
			Timeout: rpcTimeout,
		})
		if err != nil {
	...
	for _, file := range allFiles {
		contextData := ainshrpc.CommandWaveAIAddContextData{
			Files: []ainshrpc.AIAttachedFile{file},
		}
		err := wshclient.AinAiAddContextCommand(RpcClient, contextData, &ainshrpc.RpcOpts{
			Route:   route,
			Timeout: rpcTimeout,
		})
		if err != nil {
	...
	if aiMessageFlag != "" || aiSubmitFlag {
		finalContextData := ainshrpc.CommandWaveAIAddContextData{
			Text:   aiMessageFlag,
			Submit: aiSubmitFlag,
		}
		err := wshclient.AinAiAddContextCommand(RpcClient, finalContextData, &ainshrpc.RpcOpts{
			Route:   route,
			Timeout: rpcTimeout,
		})
