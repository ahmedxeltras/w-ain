...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/util/colprint"
...
func fileInfoRun(cmd *cobra.Command, args []string) error {
	...
	info, err := wshclient.FileInfoCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: fileTimeout})
...
func fileRmRun(cmd *cobra.Command, args []string) error {
	...
	err = wshclient.FileDeleteCommand(RpcClient, ainshrpc.CommandDeleteFileData{Path: path, Recursive: recursive}, &ainshrpc.RpcOpts{Timeout: fileTimeout})
	if err != nil {
...
func fileWriteRun(cmd *cobra.Command, args []string) error {
	...
	capability, err := wshclient.FileShareCapabilityCommand(RpcClient, fileData.Info.Path, &ainshrpc.RpcOpts{Timeout: fileTimeout})
	if err != nil {
	...
	if capability.CanAppend {
		...
		err = wshclient.FileWriteCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: fileTimeout})
		if err != nil {
			return fmt.Errorf("writing file: %w", err)
		}
	}
...
func fileAppendRun(cmd *cobra.Command, args []string) error {
	...
	for {
		...
		if buf.Len() >= 8192 { // 8KB batch size
			...
			err = wshclient.FileAppendCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: fileTimeout})
			if err != nil {
...
func fileCpRun(cmd *cobra.Command, args []string) error {
	...
	rpcOpts := &ainshrpc.RpcOpts{Timeout: TimeoutYear}
	err = wshclient.FileCopyCommand(RpcClient, ainshrpc.CommandFileCopyData{SrcUri: srcPath, DestUri: destPath, Opts: &ainshrpc.FileCopyOpts{Merge: merge, Overwrite: force, Timeout: TimeoutYear}}, rpcOpts)
	if err != nil {
...
func fileMvRun(cmd *cobra.Command, args []string) error {
	...
	rpcOpts := &ainshrpc.RpcOpts{Timeout: TimeoutYear}
	err = wshclient.FileMoveCommand(RpcClient, ainshrpc.CommandFileCopyData{SrcUri: srcPath, DestUri: destPath, Opts: &ainshrpc.FileCopyOpts{Overwrite: force, Timeout: TimeoutYear, Recursive: recursive}}, rpcOpts)
	if err != nil {
...
func filePrintColumns(filesChan <-chan ainshrpc.RespOrErrorUnion[ainshrpc.CommandRemoteListEntriesRtnData]) error {
	...
	return colprint.PrintColumnsArray(
		...
		numCols,
		100, // sample size
		func(respUnion ainshrpc.RespOrErrorUnion[ainshrpc.CommandRemoteListEntriesRtnData]) ([]string, error) {
...
func filePrintLong(filesChan <-chan ainshrpc.RespOrErrorUnion[ainshrpc.CommandRemoteListEntriesRtnData]) error {
	// Sample first 100 files to determine name width
	maxNameLen := 0
	var samples []*ainshrpc.FileInfo
...
func fileListRun(cmd *cobra.Command, args []string) error {
	...
	filesChan := wshclient.FileListStreamCommand(RpcClient, ainshrpc.FileListData{Path: path, Opts: &ainshrpc.FileListOpts{All: recursive}}, &ainshrpc.RpcOpts{Timeout: 2000})
