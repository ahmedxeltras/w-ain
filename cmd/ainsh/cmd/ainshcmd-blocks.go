...
type BlockDetails struct {
	...
	Meta        ainobj.MetaMapType `json:"meta"`        // Block metadata including view type
}
...
func blocksListRun(cmd *cobra.Command, args []string) error {
	...
	workspaces, err := wshclient.WorkspaceListCommand(RpcClient, &ainshrpc.RpcOpts{Timeout: int64(blocksTimeout)})
	if err != nil {
	...
	for _, wsId := range workspaceIdsToQuery {
		req := ainshrpc.BlocksListRequest{WorkspaceId: wsId}
		if blocksWindowId != "" {
			req.WindowId = blocksWindowId
		}

		blocks, err := wshclient.BlocksListCommand(RpcClient, req, &ainshrpc.RpcOpts{Timeout: int64(blocksTimeout)})
		if err != nil {
		...
		for _, b := range blocks {
			...
			if blocksView != "" {
				view := b.Meta.GetString(ainobj.MetaKey_View, "")
			...
			v := b.Meta.GetString(ainobj.MetaKey_View, "")
	...
	for _, b := range allBlocks {
		...
		switch view {
		case "preview", "edit":
			content = b.Meta.GetString(ainobj.MetaKey_File, "<no file>")
		case "web":
			content = b.Meta.GetString(ainobj.MetaKey_Url, "<no url>")
		case "term":
			content = b.Meta.GetString(ainobj.MetaKey_CmdCwd, "<no cwd>")
		default:
			content = ""
		}
