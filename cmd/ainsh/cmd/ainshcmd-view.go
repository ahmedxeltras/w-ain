...
func viewRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	var wshCmd *ainshrpc.CommandCreateBlockData
	if strings.HasPrefix(fileArg, "http://") || strings.HasPrefix(fileArg, "https://") {
		...
		if cmdName == "edit" {
			wshCmd.BlockDef.Meta[ainobj.MetaKey_Edit] = true
		}
		if conn != "" {
			wshCmd.BlockDef.Meta[ainobj.MetaKey_Connection] = conn
		}
	}
