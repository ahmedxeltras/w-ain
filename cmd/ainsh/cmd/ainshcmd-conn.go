...
func getAllConnStatus() ([]ainshrpc.ConnStatus, error) {
	var allResp []ainshrpc.ConnStatus
...
func connReinstallRun(cmd *cobra.Command, args []string) error {
	...
	data := ainshrpc.ConnExtData{
		ConnName:   connName,
		LogBlockId: RpcContext.BlockId,
	}
	err := wshclient.ConnReinstallWshCommand(RpcClient, data, &ainshrpc.RpcOpts{Timeout: 60000})
	if err != nil {
...
func connDisconnectRun(cmd *cobra.Command, args []string) error {
	...
	err := wshclient.ConnDisconnectCommand(RpcClient, connName, &ainshrpc.RpcOpts{Timeout: 10000})
	if err != nil {
...
func connDisconnectAllRun(cmd *cobra.Command, args []string) error {
	...
	for _, conn := range allConns {
		...
		err := wshclient.ConnDisconnectCommand(RpcClient, conn.Connection, &ainshrpc.RpcOpts{Timeout: 10000})
		if err != nil {
...
func connConnectRun(cmd *cobra.Command, args []string) error {
	...
	data := ainshrpc.ConnRequest{
		Host:       connName,
		LogBlockId: RpcContext.BlockId,
	}
	err := wshclient.ConnConnectCommand(RpcClient, data, &ainshrpc.RpcOpts{Timeout: 60000})
	if err != nil {
...
func connEnsureRun(cmd *cobra.Command, args []string) error {
	...
	err := wshclient.ConnEnsureCommand(RpcClient, data, &ainshrpc.RpcOpts{Timeout: 60000})
	if err != nil {
