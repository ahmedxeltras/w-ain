...
func getVarRun(cmd *cobra.Command, args []string) error {
	...
	key := args[0]
	commandData := ainshrpc.CommandVarData{
		Key:      key,
		ZoneId:   fullORef.OID,
		FileName: getVarFileName,
	}

	resp, err := wshclient.GetVarCommand(RpcClient, commandData, &ainshrpc.RpcOpts{Timeout: 2000})
	if err != nil {
...
func getAllVariables(zoneId string) error {
	...
	data, err := wshclient.FileReadCommand(RpcClient, fileData, &ainshrpc.RpcOpts{Timeout: 2000})
