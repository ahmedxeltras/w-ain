...
func setBgRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	if len(args) == 0 {
		...
		if strings.HasPrefix(input, "#") {
			...
			// Handle image input
			absPath, err := filepath.Abs(ainbase.ExpandHomeDirSafe(input))
			if err != nil {
	...
	// Send RPC request
	setMetaWshCmd := ainshrpc.CommandSetMetaData{
		ORef: *oRef,
		Meta: meta,
	}
	err = wshclient.SetMetaCommand(RpcClient, setMetaWshCmd, &ainshrpc.RpcOpts{Timeout: 2000})
	if err != nil {
