...
func secretGetRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	resp, err := wshclient.GetSecretsCommand(RpcClient, []string{name}, &ainshrpc.RpcOpts{Timeout: 2000})
	if err != nil {
...
func secretSetRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	backend, err := wshclient.GetSecretsLinuxStorageBackendCommand(RpcClient, &ainshrpc.RpcOpts{Timeout: 2000})
	if err != nil {
	...
	err = wshclient.SetSecretsCommand(RpcClient, secrets, &ainshrpc.RpcOpts{Timeout: 2000})
	if err != nil {
...
func secretListRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	names, err := wshclient.GetSecretsNamesCommand(RpcClient, &ainshrpc.RpcOpts{Timeout: 2000})
	if err != nil {
