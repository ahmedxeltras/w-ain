...
const AIToolUseBatch = memo(({ parts, isStreaming }: AIToolUseBatchProps) => {
    ...
    const handleApprove = () => {
        ...
        parts.forEach((part) => {
            AinAiModel.getInstance().toolUseSendApproval(part.data.toolcallid, "user-approved");
        });
    };
    ...
    const handleDeny = () => {
        ...
        parts.forEach((part) => {
            AinAiModel.getInstance().toolUseSendApproval(part.data.toolcallid, "user-denied");
        });
    };
...
const AIToolUse = memo(({ part, isStreaming }: AIToolUseProps) => {
    ...
    const handleApprove = () => {
        setUserApprovalOverride("user-approved");
        AinAiModel.getInstance().toolUseSendApproval(toolData.toolcallid, "user-approved");
    };

    const handleDeny = () => {
        setUserApprovalOverride("user-denied");
        AinAiModel.getInstance().toolUseSendApproval(toolData.toolcallid, "user-denied");
    };
    ...
    const handleOpenDiff = () => {
        recordTEvent("ainai:showdiff");
        fireAndForget(() => AinAiModel.getInstance().openDiff(toolData.inputfilename, toolData.toolcallid));
    };
            ...
            <div className="flex items-center gap-2">
                ...
                {isFileWriteTool &&
                    ...
                    Date.now() - toolData.runts < BackupRetentionDays * 24 * 60 * 60 * 1000 && (
                        <button
                            onClick={() => {
                                recordTEvent("ainai:revertfile", { "ainai:action": "revertfile:open" });
