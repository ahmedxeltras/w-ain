...
import { handleAinAiContextMenu } from "@/app/aipanel/aipanel-contextmenu";
...
const AIWelcomeMessage = memo(() => {
    ...
    const aiModeConfigs = jotai.useAtomValue(atoms.ainAiModeConfigAtom);
    const hasCustomModes = Object.keys(aiModeConfigs).some((key) => !key.startsWith("ainai@"));
    return (
        <div className="text-secondary py-8">
            <div className="text-center">
                <i className="fa fa-sparkles text-4xl text-accent mb-2 block"></i>
                <p className="text-lg font-bold text-primary">Welcome to Ain AI</p>
            </div>
            <div className="mt-4 text-left max-w-md mx-auto">
                <p className="text-sm mb-6">
                    Ain AI is your terminal assistant with context. I can read your terminal output, analyze widgets,
    ...
    return (
        <div className="text-secondary py-8">
            <div className="text-center">
                ...
                <p className="text-lg font-bold text-primary">AinApp Builder</p>
            </div>
            <div className="mt-4 text-left max-w-md mx-auto">
                <p className="text-sm mb-6">
                    The AinApp builder helps create wave widgets that integrate seamlessly into Ain Term.
                </p>
            </div>
        </div>
    );
...
const AIPanelComponentInner = memo(() => {
    ...
    const isFocused = jotai.useAtomValue(model.isAinAiFocusedAtom);
    const telemetryEnabled = jotai.useAtomValue(getSettingsKeyAtom("telemetry:enabled")) ?? false;
    const isPanelVisible = jotai.useAtomValue(model.getPanelVisibleAtom());
    const tabModel = maybeUseTabModel();
    const defaultMode = jotai.useAtomValue(getSettingsKeyAtom("ainai:defaultmode")) ?? "ainai@balanced";
    const aiModeConfigs = jotai.useAtomValue(model.aiModeConfigs);

    const hasCustomModes = Object.keys(aiModeConfigs).some((key) => !key.startsWith("ainai@"));
    const isUsingCustomMode = !defaultMode.startsWith("ainai@");
    ...
    const handleFocusCapture = useCallback(
        (event: React.FocusEvent) => {
            // console.log("Ain AI focus capture", getElemAsStr(event.target));
                    ...
                    <>
                        {messages.length === 0 && initialLoadDone ? (
                            <div
                                ...
                                onContextMenu={(e) => handleAinAiContextMenu(e, true)}
                            >
