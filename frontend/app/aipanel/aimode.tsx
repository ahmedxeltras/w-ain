...
export const AIModeDropdown = memo(({ compatibilityMode = false }: AIModeDropdownProps) => {
    ...
    const ainaiModeConfigs = useAtomValue(atoms.ainAiModeConfigAtom);
    const widgetContextEnabled = useAtomValue(model.widgetAccessAtom);
    const hasPremium = useAtomValue(model.hasPremiumAtom);
    const showCloudModes = useAtomValue(getSettingsKeyAtom("ainai:showcloudmodes"));
    ...
    const handleSelect = (mode: string) => {
        ...
        if (!config) return;
        if (!hasPremium && config["ainai:premium"]) {
            return;
        }
    ...
    const resolvedConfig = ainaiModeConfigs[currentMode];
                ...
                <Tooltip
                    content={
                        <div className="max-w-xs">
                            ...
                            "ai:capabilities" array. Without tool support, Ain AI will not be able to interact with
                            widgets or files.
                        </div>
                    }
                    placement="bottom"
                            ...
                            return (
                                <div key={section.sectionName}>
                                    ...
                                    {showSectionHeaders && (
                                        <>
                                            ...
                                            {section.noTelemetry && (
                                                ...
                                                >
                                                    (enable telemetry to unlock Ain AI Cloud)
                                                </button>
                                            )}
                                        </>
                                    )}
                                    {section.configs.map((config, index) => {
                                        ...
                                        const isPremiumDisabled = !hasPremium && config["ainai:premium"];
