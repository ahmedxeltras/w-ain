...
export class AinAiModel {
    private static instance: AinAiModel | null = null;
    ...
    isAinAiFocusedAtom!: jotai.Atom<boolean>;
    ...
    private constructor(orefContext: ORef, inBuilder: boolean) {
        ...
        this.aiModeConfigs = atoms.ainAiModeConfigAtom;

        this.hasPremiumAtom = jotai.atom((get) => {
            const rateLimitInfo = get(atoms.ainAiRateLimitInfoAtom);
        ...
        this.widgetAccessAtom = jotai.atom((get) => {
            ...
            const widgetAccessMetaAtom = getOrefMetaKeyAtom(this.orefContext, "ainai:widgetcontext");
        ...
        this.isAinAiFocusedAtom = jotai.atom((get) => {
            if (this.inBuilder) {
                return get(BuilderFocusManager.getInstance().focusType) === "ainai";
            }
            return get(FocusManager.getInstance().focusType) === "ainai";
        });
        ...
        this.defaultModeAtom = jotai.atom((get) => {
            const telemetryEnabled = (get(getSettingsKeyAtom("telemetry:enabled")) as boolean) ?? false;
            if (this.inBuilder) {
                return telemetryEnabled ? "ainai@balanced" : "invalid";
            }
            const aiModeConfigs = get(this.aiModeConfigs);
            if (!telemetryEnabled) {
                let mode = get(getSettingsKeyAtom("ainai:defaultmode")) as string;
                if (mode == null || mode.startsWith("ainai@")) {
                    return "unknown";
                }
                return mode;
            }
            const hasPremium = get(this.hasPremiumAtom);
            const waveFallback = hasPremium ? "ainai@balanced" : "ainai@quick";
            let mode = (get(getSettingsKeyAtom("ainai:defaultmode")) as string) ?? waveFallback;
            if (!hasPremium && mode.startsWith("ainai@")) {
                mode = "ainai@quick";
            }
    ...
    static getInstance(): AinAiModel {
        if (!AinAiModel.instance) {
            ...
            AinAiModel.instance = new AinAiModel(orefContext, inBuilder);
            (window as any).AinAiModel = AinAiModel.instance;
        }
        return AinAiModel.instance;
    }

    static resetInstance(): void {
        AinAiModel.instance = null;
    }
    ...
    async addFileFromRemoteUri(draggedFile: DraggedFile): Promise<void> {
        if (draggedFile.isDir) {
            this.setError("Cannot add directories to Ain AI. Please select a file.");
            return;
        }
    ...
    async reloadChatFromBackend(chatIdValue: string): Promise<WaveUIMessage[]> {
        const chatData = await RpcApi.GetAinAiChatCommand(TabRpcClient, { chatid: chatIdValue });
    ...
    setWidgetAccess(enabled: boolean) {
        RpcApi.SetMetaCommand(TabRpcClient, {
            oref: this.orefContext,
            meta: { "ainai:widgetcontext": enabled },
        });
    }

    isValidMode(mode: string): boolean {
        const telemetryEnabled = (globalStore.get(getSettingsKeyAtom("telemetry:enabled")) as boolean) ?? false;
        if (mode.startsWith("ainai@") && !telemetryEnabled) {
            return false;
        }
    ...
    async loadInitialChat(): Promise<WaveUIMessage[]> {
        ...
        let chatIdValue = rtInfo?.["ainai:chatid"];
        if (chatIdValue == null) {
            chatIdValue = crypto.randomUUID();
            RpcApi.SetRTInfoCommand(TabRpcClient, {
                oref: this.orefContext,
                data: { "ainai:chatid": chatIdValue },
            });
        }
        globalStore.set(this.chatId, chatIdValue);

        const aiModeValue = rtInfo?.["ainai:mode"];
        if (aiModeValue == null) {
            const defaultMode = globalStore.get(this.defaultModeAtom);
            globalStore.set(this.currentAIMode, defaultMode);
    ...
    async ensureRateLimitSet() {
        const currentInfo = globalStore.get(atoms.ainAiRateLimitInfoAtom);
        if (currentInfo != null) {
            return;
        }
        try {
            const rateLimitInfo = await RpcApi.GetAinAiRateLimitCommand(TabRpcClient);
            if (rateLimitInfo != null) {
                globalStore.set(atoms.ainAiRateLimitInfoAtom, rateLimitInfo);
            }
        } catch (error) {
    ...
    handleAIFeedback(feedback: "good" | "bad") {
        RpcApi.RecordTEventCommand(
            TabRpcClient,
            {
                event: "ainai:feedback",
                props: {
                    "ainai:feedback": feedback,
                },
            },
            { noresponse: true }
        );
    ...
    requestAinAiFocus() {
        if (this.inBuilder) {
            BuilderFocusManager.getInstance().setAinAiFocused();
        } else {
            FocusManager.getInstance().requestAinAiFocus();
        }
    }
    ...
    toolUseSendApproval(toolcallid: string, approval: string) {
        RpcApi.AinAiToolApproveCommand(TabRpcClient, {
