...
function initGlobalAtoms(initOpts: GlobalInitOptions) {
    ...
    const ainAiModeConfigAtom = atom(null) as PrimitiveAtom<Record<string, AIModeConfigType>>;
    ...
    atoms = {
        ...
        reinitVersion,
        ainAiRateLimitInfoAtom: rateLimitInfoAtom,
    } as GlobalAtomsType;
}

function initGlobalWaveEventSubs(initOpts: AinInitOpts) {
    waveEventSubscribe(
        {
        ...
        {
            eventType: "ainai:modeconfig",
            handler: (event) => {
                const modeConfigs = (event.data as AIModeConfigUpdate).configs;
                globalStore.set(atoms.ainAiModeConfigAtom, modeConfigs);
            },
        },
        {
            eventType: "userinput",
