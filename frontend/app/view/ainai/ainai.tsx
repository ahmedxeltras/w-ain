...
function promptToMsg(prompt: AinAiPromptMessageType): ChatMessageType {
    return {
...
class AiWshClient extends WshClient {
    ...
    constructor(blockId: string, model: AinAiModel) {
...
export class AinAiModel implements ViewModel {
    viewType: string;
    blockId: string;
                ...
                default:
                    if (isCloud) {
                        viewTextChildren.push({
                            elemtype: "iconbutton",
                            icon: "cloud",
                            title: "Using Ain AI Proxy (gpt-5-mini)",
                            noAction: true,
                        });
                    } else {
    ...
    async fetchAiData(): Promise<Array<AinAiPromptMessageType>> {
        const { data } = await fetchWaveFile(this.blockId, "aidata");
        if (!data) {
            return [];
        }
        const history: Array<AinAiPromptMessageType> = JSON.parse(new TextDecoder().decode(data));
    ...
    sendMessage(text: string, user: string = "user") {
        ...
        const newPrompt: AinAiPromptMessageType = {
            role: "user",
            content: text,
        };
        const handleAiStreamingResponse = async () => {
            ...
            const history = await this.fetchAiData();
            const beMsg: AinAiStreamRequest = {
                clientid: clientId,
                opts,
                prompt: history.concat(newPrompt),
            };
            let fullMsg = "";
            try {
                const aiGen = RpcApi.StreamAinAiCommand(TabRpcClient, beMsg, { timeout: opts.timeoutms });
                ...
                if (fullMsg == "") {
                    ...
                    await BlockService.SaveAinAiData(this.blockId, [...history, newPrompt]);
                } else {
                    const responsePrompt: AinAiPromptMessageType = {
                        role: "assistant",
                        content: fullMsg,
                    };
                    ...
                    await BlockService.SaveAinAiData(this.blockId, [...history, newPrompt, responsePrompt]);
                }
            } catch (error) {
                ...
                globalStore.set(this.updateLastMessageAtom, "", false);
                const errorPrompt: AinAiPromptMessageType = {
                    role: "error",
                    content: errMsg,
                };
                updatedHist.push(errorPrompt);
                await BlockService.SaveAinAiData(this.blockId, updatedHist);
            }
    ...
    async clearMessages() {
        await BlockService.SaveAinAiData(this.blockId, []);
...
const AinAi = ({ model }: { model: AinAiModel; blockId: string }) => {
    const { sendMessage } = model.useAinAi();
    const ainAiRef = useRef<HTMLDivElement>(null);
    ...
    let buttonClass = "ainai-submit-button";
    let buttonIcon = makeIconClass("arrow-up", false);
    let buttonTitle = "run";
    if (locked) {
        buttonClass = "ainai-submit-button stop";
    ...
    return (
        <div ref={ainAiRef} className="ainai">
            {isUsingProxy && (
                <div className="flex items-start gap-3 px-4 py-2 bg-orange-500/25 border-b border-orange-500/50 text-sm">
                    ...
                    <span className="text-primary/90">
                        Ain AI Proxy is deprecated and will be removed. Please use the new{" "}
                        <button
            ...
            <div className="ainai-chat">
                <ChatWindow ref={osRef} chatWindowRef={chatWindowRef} msgWidths={msgWidths} model={model} />
            </div>
            <div className="ainai-controls">
                <div className="ainai-input-wrapper">
                    <ChatInput
                        ref={inputRef}
                        value={value}
                        model={model}
