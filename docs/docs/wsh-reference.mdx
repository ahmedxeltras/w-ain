...
The `web` command opens URLs in a web block within Ain Term.
...
wsh web open "Ain Term documentation"
...
The `notify` command creates a desktop notification from Ain Term.
...
Ain Term is capable of managing files from remote SSH hosts, S3-compatible
...
Ain Term provides commands for managing persistent variables at different scopes (block, tab, workspace, or client-wide).
...
The `wavepath` command lets you get the paths to various Ain Term directories and files, including configuration, data storage, and logs.

```sh
wsh wavepath {config|data|log}
```

This command returns the full path to the requested Ain Term system directory or file. It's useful for accessing Wave's configuration files, data storage, or checking logs.
...
- `config` - Where Ain Term stores its configuration files
- `data` - Where Ain Term stores its persistent data
- `log` - The main Ain Term log file
...
- `--view <type>` - filter by view type (term, web, preview, edit, sysinfo, ainai)
