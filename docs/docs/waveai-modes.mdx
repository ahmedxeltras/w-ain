...
AI modes are configured in `~/.config/waveterm/ainai.json`.
...
You can configure global Wave AI behavior in your Ain Term settings (separate from the mode configurations in `ainai.json`).

### Setting a Default AI Mode

After configuring a local model or custom mode, you can make it the default by setting `ainai:defaultmode` in your Ain Term settings.

:::important
Use the **mode key** (the key in your `ainai.json` configuration), not the display name. For example, use `"ollama-llama"` (the key), not `"Ollama - Llama 3.3"` (the display name).
:::

**Using the settings command:**
```bash
wsh setconfig ainai:defaultmode="ollama-llama"
```

**Or edit settings.json directly:**
1. Click the settings (gear) icon in the widget bar
2. Select "Settings" from the menu
3. Add the `ainai:defaultmode` key to your settings.json:
```json
  "ainai:defaultmode": "ollama-llama"
```

This will make the specified mode the default selection when opening Wave AI features.

:::note
Wave AI normally requires telemetry to be enabled. However, if you configure your own custom model (local or BYOK) and set `ainai:defaultmode` to that custom mode's key, you will not receive telemetry requirement messages. This allows you to use Wave AI features completely privately with your own models. <VersionBadge version="v0.13.1"/>
:::

### Hiding Wave Cloud Modes

If you prefer to use only your local or custom models and want to hide Wave's cloud AI modes from the mode dropdown, set `ainai:showcloudmodes` to `false`:

**Using the settings command:**
```bash
wsh setconfig ainai:showcloudmodes=false
```

**Or edit settings.json directly:**
1. Click the settings (gear) icon in the widget bar
2. Select "Settings" from the menu
3. Add the `ainai:showcloudmodes` key to your settings.json:
```json
  "ainai:showcloudmodes": false
```
...
| `ainai:cloud` | No | Internal - for Wave Cloud AI configuration only |
| `ainai:premium` | No | Internal - for Wave Cloud AI configuration only |
...
:::warning
If you don't include `"tools"` in the `ai:capabilities` array, the AI model will not be able to interact with your Ain Term widgets, read/write files, or execute commands. Most AI modes should include `"tools"` for the best Wave experience.
:::
