...
Ain Term collects telemetry data to help us track feature use, direct future product efforts, and generate aggregate metrics on Wave's popularity and usage. We do NOT collect personal information (PII), keystrokes, file contents, AI prompts, IP addresses, hostnames, or commands. We attach all information to an anonymous, randomly generated _ClientId_ (UUID). You may opt out of collection at any time.
...
This document outlines the current telemetry system as of v0.11.1. As of v0.12.5, Ain Term no longer sends legacy telemetry. The previous telemetry documentation can be found in our [Legacy Telemetry Documentation](./telemetry-old.mdx) for historical reference.
...
This ping is intentionally separate from telemetry so Wave can count active installs. If you'd like to disable it, set the AINTERM_NOPING environment variable.
...
For a summary of the above, you can take a look at our [Privacy Policy](https://www.ainterm.dev/privacy).
