...
func FindGitBash(config *ainconfig.FullConfigType, rescan bool) string {
	if runtime.GOOS != "windows" {
		return ""
	}
...
func DefaultTermSize() ainobj.TermSize {
	return ainobj.TermSize{Rows: DefaultTermRows, Cols: DefaultTermCols}
}
...
func WaveshellLocalEnvVars(termType string) map[string]string {
	...
	rtn["AINTERM_VERSION"] = ainbase.WaveVersion
	rtn["AINTERM_WSHBINDIR"] = filepath.Join(ainbase.GetWaveDataDir(), WaveHomeBinDir)
	return rtn
}
...
func GetLocalBashRcFileOverride() string {
	return filepath.Join(ainbase.GetWaveDataDir(), BashIntegrationDir, ".bashrc")
}

func GetLocalWaveFishFilePath() string {
	return filepath.Join(ainbase.GetWaveDataDir(), FishIntegrationDir, "wave.fish")
}

func GetLocalWavePowershellEnv() string {
	return filepath.Join(ainbase.GetWaveDataDir(), PwshIntegrationDir, "wavepwsh.ps1")
}

func GetLocalZshZDotDir() string {
	return filepath.Join(ainbase.GetWaveDataDir(), ZshIntegrationDir)
}
...
func GetLocalWshBinaryPath(version string, goos string, goarch string) (string, error) {
	...
	if !ainbase.SupportedWshBinaries[fmt.Sprintf("%s-%s", goos, goarch)] {
		return "", fmt.Errorf("unsupported wsh platform: %s-%s", goos, goarch)
	}
	baseName := fmt.Sprintf("wsh-%s-%s.%s%s", version, goos, goarch, ext)
	return filepath.Join(ainbase.GetWaveAppBinPath(), baseName), nil
}
...
func InitRcFiles(waveHome string, absWshBinDir string) error {
	...
	err := ainbase.CacheEnsureDir(zshDir, ZshIntegrationDir, 0755, ZshIntegrationDir)
	if err != nil {
		return err
	}
	bashDir := filepath.Join(waveHome, BashIntegrationDir)
	err = ainbase.CacheEnsureDir(bashDir, BashIntegrationDir, 0755, BashIntegrationDir)
	if err != nil {
		return err
	}
	fishDir := filepath.Join(waveHome, FishIntegrationDir)
	err = ainbase.CacheEnsureDir(fishDir, FishIntegrationDir, 0755, FishIntegrationDir)
	if err != nil {
		return err
	}
	pwshDir := filepath.Join(waveHome, PwshIntegrationDir)
	err = ainbase.CacheEnsureDir(pwshDir, PwshIntegrationDir, 0755, PwshIntegrationDir)
	if err != nil {
		return err
	}
...
func initCustomShellStartupFilesInternal() error {
	...
	waveDataHome := ainbase.GetWaveDataDir()
	...
	err = ainbase.CacheEnsureDir(binDir, WaveHomeBinDir, 0755, WaveHomeBinDir)
	if err != nil {
		return err
	}

	// copy the correct binary to bin
	wshFullPath, err := GetLocalWshBinaryPath(ainbase.WaveVersion, runtime.GOOS, runtime.GOARCH)
	if err != nil {
