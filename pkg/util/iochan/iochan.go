...
func ReaderChan(ctx context.Context, r io.Reader, chunkSize int64, callback func()) chan ainshrpc.RespOrErrorUnion[iochantypes.Packet] {
	ch := make(chan ainshrpc.RespOrErrorUnion[iochantypes.Packet], 32)
	go func() {
		defer func() {
			...
			default:
				...
				if n, err := r.Read(buf); err != nil {
					if errors.Is(err, io.EOF) {
						ch <- ainshrpc.RespOrErrorUnion[iochantypes.Packet]{Response: iochantypes.Packet{Checksum: sha256Hash.Sum(nil)}} // send the checksum
						return
					}
					ch <- ainshutil.RespErr[iochantypes.Packet](fmt.Errorf("ReaderChan: read error: %v", err))
					return
				} else if n > 0 {
					if _, err := sha256Hash.Write(buf[:n]); err != nil {
						ch <- ainshutil.RespErr[iochantypes.Packet](fmt.Errorf("ReaderChan: error writing to sha256 hash: %v", err))
						return
					}
					ch <- ainshrpc.RespOrErrorUnion[iochantypes.Packet]{Response: iochantypes.Packet{Data: buf[:n]}}
				}
			}
		}
	}()
...
func WriterChan(ctx context.Context, w io.Writer, ch <-chan ainshrpc.RespOrErrorUnion[iochantypes.Packet], callback func(), cancel context.CancelCauseFunc) {
	go func() {
		defer func() {
			if ctx.Err() != nil {
