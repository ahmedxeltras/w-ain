...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/ijson"
...
type WaveFile struct {
	...
	Opts      ainshrpc.FileOpts `json:"opts"`
	CreatedTs int64             `json:"createdts"`

	//  these fields are mutable
	Size  int64             `json:"size"`
	ModTs int64             `json:"modts"`
	Meta  ainshrpc.FileMeta `json:"meta"` // only top-level keys can be updated (lower levels are immutable)
}
...
func copyMeta(meta ainshrpc.FileMeta) ainshrpc.FileMeta {
	newMeta := make(ainshrpc.FileMeta)
	for k, v := range meta {
		newMeta[k] = v
	}
	return newMeta
...
func (s *FileStore) MakeFile(ctx context.Context, zoneId string, name string, meta ainshrpc.FileMeta, opts ainshrpc.FileOpts) error {
	if opts.MaxSize < 0 {
...
func (s *FileStore) WriteMeta(ctx context.Context, zoneId string, name string, meta ainshrpc.FileMeta, merge bool) error {
	return withLock(s, zoneId, name, func(entry *CacheEntry) error {
...
func metaIncrement(file *WaveFile, key string, amount int) int {
	if file.Meta == nil {
		file.Meta = make(ainshrpc.FileMeta)
	}
