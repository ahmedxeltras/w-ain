...
func processWSCommand(jmsg map[string]any, outputCh chan any, rpcInputCh chan baseds.RpcInputChType) {
	...
	case *webcmd.SetBlockTermSizeWSCommand:
		data := ainshrpc.CommandBlockInputData{
			BlockId:  cmd.BlockId,
			TermSize: &cmd.TermSize,
		}
		rpcMsg := ainshutil.RpcMessage{
			Command: ainshrpc.Command_ControllerInput,
			Data:    data,
		}
...
func registerConn(wsConnId string, stableId string, wproxy *ainshutil.WshRpcProxy) {
	GlobalLock.Lock()
	...
	if curConnInfo != nil {
		...
		if curConnInfo.LinkId != baseds.NoLinkId {
			ainshutil.DefaultRouter.UnregisterLink(curConnInfo.LinkId)
		}
	}
	linkId := ainshutil.DefaultRouter.RegisterTrustedRouter(wproxy)
...
func HandleWsInternal(w http.ResponseWriter, r *http.Request) error {
	...
	wproxy := ainshutil.MakeRpcProxy(fmt.Sprintf("ws:%s", stableId))
