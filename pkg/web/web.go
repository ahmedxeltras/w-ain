...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/schema"
...
func handleRemoteStreamFile(w http.ResponseWriter, req *http.Request, conn string, path string, no404 bool) error {
	...
	streamFileData := ainshrpc.CommandRemoteStreamFileData{Path: path}
	route := ainshutil.MakeConnectionRouteId(conn)
	rpcOpts := &ainshrpc.RpcOpts{Route: route, Timeout: 60 * 1000}
...
func handleRemoteStreamFileFromCh(w http.ResponseWriter, req *http.Request, path string, rtnCh <-chan ainshrpc.RespOrErrorUnion[ainshrpc.FileData], streamCancelFn func(context.Context) error, no404 bool) error {
	firstPk := true
	var fileInfo *ainshrpc.FileInfo
	loopDone := false
	defer func() {
		if loopDone {
			return
...
func MakeUnixListener() (net.Listener, error) {
	serverAddr := ainbase.GetDomainSocketName()
