...
type mockRpcInterface struct {
	dataChan chan ainshrpc.CommandStreamData
	ackChan  chan ainshrpc.CommandStreamAckData
}

func (m *mockRpcInterface) StreamDataCommand(data ainshrpc.CommandStreamData, opts *ainshrpc.RpcOpts) error {
	m.dataChan <- data
	return nil
}

func (m *mockRpcInterface) StreamDataAckCommand(data ainshrpc.CommandStreamAckData, opts *ainshrpc.RpcOpts) error {
	m.ackChan <- data
	return nil
}
...
func setupBrokerPair() (*Broker, *Broker) {
	rpc1 := &mockRpcInterface{
		dataChan: make(chan ainshrpc.CommandStreamData, 10),
		ackChan:  make(chan ainshrpc.CommandStreamAckData, 10),
	}
