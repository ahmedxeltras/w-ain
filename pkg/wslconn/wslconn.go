...
func (conn *WslConn) DeriveConnStatus() ainshrpc.ConnStatus {
	conn.Lock.Lock()
...
func (conn *WslConn) OpenDomainSocketListener(ctx context.Context) error {
	...
	*/
	conn.Infof(ctx, "setting domain socket to %s\n", ainbase.RemoteFullDomainSocketPath)
	conn.WithLock(func() {
		conn.DomainSockName = ainbase.RemoteFullDomainSocketPath
...
func (conn *WslConn) StartConnServer(ctx context.Context, afterUpdate bool) (bool, string, string, error) {
	...
	go func() {
		...
		ainshutil.HandleStdIOClient(logName, linesChan, inputPipeWrite)
	}()
	...
	defer cancelFn()
	err = ainshutil.DefaultRouter.WaitForRegister(regCtx, ainshutil.MakeConnectionRouteId(conn.GetName()))
	if err != nil {
...
func (conn *WslConn) UpdateWsh(ctx context.Context, clientDisplayName string, remoteInfo *ainshrpc.RemoteInfo) error {
...
func (conn *WslConn) getConnectionConfig() (ainconfig.ConnKeywords, bool) {
	config := ainconfig.GetWatcher().GetFullConfig()
