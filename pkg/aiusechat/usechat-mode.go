...
func resolveAIMode(requestedMode string, premium bool) (string, *ainconfig.AIModeConfigType, error) {
	mode := requestedMode
...
func applyProviderDefaults(config *ainconfig.AIModeConfigType) {
...
func getAIModeConfig(aiMode string) (*ainconfig.AIModeConfigType, error) {
...
func handleConfigUpdate(fullConfig ainconfig.FullConfigType) {
	resolvedConfigs := ComputeResolvedAIModeConfigs(fullConfig)
	broadcastAIModeConfigs(resolvedConfigs)
}

func ComputeResolvedAIModeConfigs(fullConfig ainconfig.FullConfigType) map[string]ainconfig.AIModeConfigType {
	resolvedConfigs := make(map[string]ainconfig.AIModeConfigType)
...
func broadcastAIModeConfigs(configs map[string]ainconfig.AIModeConfigType) {
	update := ainconfig.AIModeConfigUpdate{
		Configs: configs,
	}

	ainps.Broker.Publish(ainps.WaveEvent{
		Event: ainps.Event_AIModeConfig,
		Data:  update,
	})
}
