...
func makeTerminalBlockDesc(block *ainobj.Block) string {
...
func MakeBlockShortDesc(block *ainobj.Block) string {
	if block.Meta == nil {
		return ""
	}
...
func GenerateCurrentTabStatePrompt(blocks []*ainobj.Block, widgetAccess bool) string {
	if !widgetAccess {
	...
	systemInfo := ainbase.GetSystemSummary()
...
func generateToolsForTsunamiBlock(block *ainobj.Block) []uctypes.ToolDefinition {
	var tools []uctypes.ToolDefinition
