...
func makeTabCaptureBlockScreenshot(tabId string) func(any) (string, error) {
	return func(input any) (string, error) {
		...
		fullBlockId, err := aincore.ResolveBlockIdFromPrefix(ctx, tabId, blockIdPrefix)
		if err != nil {
			return "", err
		}
		...
		screenshotData, err := wshclient.CaptureBlockScreenshotCommand(
			rpcClient,
			ainshrpc.CommandCaptureBlockScreenshotData{BlockId: fullBlockId},
			&ainshrpc.RpcOpts{Route: ainshutil.MakeTabRouteId(tabId)},
		)
		if err != nil {
