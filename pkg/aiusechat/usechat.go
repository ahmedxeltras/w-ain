...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/aiusechat/aiutil"
	...
	"github.com/wavetermdev/ainterm/pkg/util/logutil"
	"github.com/wavetermdev/ainterm/pkg/util/utilfn"
	"github.com/wavetermdev/ainterm/pkg/web/sse"
)
...
func getSystemPrompt(model string, isBuilder bool, hasToolsCapability bool, widgetAccess bool) []string {
	if isBuilder {
		return []string{}
	}
...
func getWaveAISettings(premium bool, builderMode bool, rtInfo ainobj.ObjRTInfo, aiModeName string) (*uctypes.AIOptsType, error) {
...
func updateRateLimit(info *uctypes.RateLimitInfo) {
	...
	go func() {
		ainps.Broker.Publish(ainps.WaveEvent{
			Event: ainps.Event_WaveAIRateLimit,
			Data:  info,
		})
	}()
}
...
func WaveAIPostMessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	var rtInfo *ainobj.ObjRTInfo
	if req.TabId != "" {
		oref := ainobj.MakeORef(ainobj.OType_Tab, req.TabId)
		rtInfo = ainstore.GetRTInfo(oref)
	} else if req.BuilderId != "" {
		oref := ainobj.MakeORef(ainobj.OType_Builder, req.BuilderId)
		rtInfo = ainstore.GetRTInfo(oref)
	}
	if rtInfo == nil {
		rtInfo = &ainobj.ObjRTInfo{}
	}
	...
	client, err := ainstore.DBGetSingleton[*ainobj.Client](r.Context())
	if err != nil {
	...
	chatOpts.SystemPrompt = getSystemPrompt(chatOpts.Config.Model, chatOpts.BuilderId != "", chatOpts.Config.HasCapability(uctypes.AICapabilityTools), chatOpts.WidgetAccess)
...
func CreateWriteTextFileDiff(ctx context.Context, chatId string, toolCallId string) ([]byte, []byte, error) {
	...
	if backupFileName != "" {
		...
		expandedPath, err := ainbase.ExpandHomeDir(params.Filename)
		if err != nil {
...
func generateBuilderAppData(appId string) (string, string, string, error) {
	appGoFile := ""
	fileData, err := ainappstore.ReadAppFile(appId, "app.go")
	if err == nil {
		appGoFile = string(fileData.Contents)
	}

	staticFilesJSON := ""
	allFiles, err := ainappstore.ListAllAppFiles(appId)
	if err == nil {
	...
	platformInfo := ainbase.GetSystemSummary()
