...
func triggerBuildAndWait(builderId string, appId string) map[string]any {
	...
	rtInfo := ainstore.GetRTInfo(ainobj.MakeORef(ainobj.OType_Builder, builderId))
...
func GetBuilderWriteAppFileToolDefinition(appId string, builderId string) uctypes.ToolDefinition {
	return uctypes.ToolDefinition{
		...
		ToolAnyCallback: func(input any, toolUseData *uctypes.UIMessageDataToolUse) (any, error) {
			...
			formattedContents := ainapputil.FormatGoCode([]byte(params.Contents))
			err = ainappstore.WriteAppFile(appId, BuilderAppFileName, formattedContents)
			if err != nil {
				return nil, err
			}

			ainps.Broker.Publish(ainps.WaveEvent{
				Event:  ainps.Event_WaveAppAppGoUpdated,
				Scopes: []string{appId},
			})
...
func GetBuilderEditAppFileToolDefinition(appId string, builderId string) uctypes.ToolDefinition {
	return uctypes.ToolDefinition{
		...
		ToolAnyCallback: func(input any, toolUseData *uctypes.UIMessageDataToolUse) (any, error) {
			...
			editResults, err := ainappstore.ReplaceInAppFilePartial(appId, BuilderAppFileName, params.Edits)
			if err != nil {
				return nil, err
			}

			// ignore format errors; gofmt can fail due to compilation errors which will be caught in the build step
			ainappstore.FormatGoFile(appId, BuilderAppFileName)
...
func GetBuilderListFilesToolDefinition(appId string) uctypes.ToolDefinition {
	return uctypes.ToolDefinition{
		...
		ToolAnyCallback: func(input any, toolUseData *uctypes.UIMessageDataToolUse) (any, error) {
			result, err := ainappstore.ListAllAppFiles(appId)
			if err != nil {
				return nil, err
			}
