...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/util/pamparse"
...
func StartWslShellProcNoWsh(ctx context.Context, termSize ainobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *wslconn.WslConn) (*ShellProc, error) {
...
func StartWslShellProc(ctx context.Context, termSize ainobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *wslconn.WslConn) (*ShellProc, error) {
	client := conn.GetClient()
	conn.Infof(ctx, "WSL-NEWSESSION (StartWslShellProc)")
	connRoute := ainshutil.MakeConnectionRouteId(conn.GetName())
	rpcClient := wshclient.GetBareRpcClient()
	remoteInfo, err := wshclient.RemoteGetInfoCommand(rpcClient, &ainshrpc.RpcOpts{Route: connRoute, Timeout: 2000})
	if err != nil {
	...
	err = wshclient.RemoteInstallRcFilesCommand(rpcClient, &ainshrpc.RpcOpts{Route: connRoute, Timeout: 2000})
	if err != nil {
	...
	if cmdStr == "" {
		...
		if shellType == shellutil.ShellType_bash {
			...
			bashPath := fmt.Sprintf("~/.ainterm/%s/.bashrc", shellutil.BashIntegrationDir)
			...
			waveFishPath := fmt.Sprintf("~/.ainterm/%s/wave.fish", shellutil.FishIntegrationDir)
			carg := fmt.Sprintf(`"source %s"`, waveFishPath)
			shellOpts = append(shellOpts, "-C", carg)
		} else if shellType == shellutil.ShellType_pwsh {
			pwshPath := fmt.Sprintf("~/.ainterm/%s/wavepwsh.ps1", shellutil.PwshIntegrationDir)
	...
	if shellType == shellutil.ShellType_zsh {
		zshDir := fmt.Sprintf("~/.ainterm/%s", shellutil.ZshIntegrationDir)
	...
	if err != nil {
		...
		cmdCombined = fmt.Sprintf(`%s=%s %s`, ainbase.WaveSwapTokenVarName, packedToken, cmdCombined)
	}
	jwtToken := cmdOpts.SwapToken.Env[ainbase.WaveJwtTokenVarName]
	if jwtToken != "" && cmdOpts.ForceJwt {
		conn.Debugf(ctx, "adding JWT token to environment\n")
		cmdCombined = fmt.Sprintf(`%s=%s %s`, ainbase.WaveJwtTokenVarName, jwtToken, cmdCombined)
	}
...
func StartRemoteShellProcNoWsh(ctx context.Context, termSize ainobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *conncontroller.SSHConn) (*ShellProc, error) {
	client := conn.GetClient()
...
func StartRemoteShellProc(ctx context.Context, logCtx context.Context, termSize ainobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *conncontroller.SSHConn) (*ShellProc, error) {
	client := conn.GetClient()
	connRoute := ainshutil.MakeConnectionRouteId(conn.GetName())
	rpcClient := wshclient.GetBareRpcClient()
	remoteInfo, err := wshclient.RemoteGetInfoCommand(rpcClient, &ainshrpc.RpcOpts{Route: connRoute, Timeout: 2000})
...
func StartLocalShellProc(logCtx context.Context, termSize ainobj.TermSize, cmdStr string, cmdOpts CommandOptsType, connName string) (*ShellProc, error) {
	...
	if err != nil {
		...
		shellutil.UpdateCmdEnv(ecmd, map[string]string{ainbase.WaveSwapTokenVarName: packedToken})
	}
	jwtToken := cmdOpts.SwapToken.Env[ainbase.WaveJwtTokenVarName]
	if jwtToken != "" && cmdOpts.ForceJwt {
		blocklogger.Debugf(logCtx, "adding JWT token to environment\n")
		shellutil.UpdateCmdEnv(ecmd, map[string]string{ainbase.WaveJwtTokenVarName: jwtToken})
	}
...
func RunSimpleCmdInPty(ecmd *exec.Cmd, termSize ainobj.TermSize) ([]byte, error) {
	ecmd.Env = os.Environ()
...
func tryGetPamEnvVars() map[string]string {
	...
	envVars3, err := pamparse.ParseEnvironmentConfFile(ainbase.ExpandHomeDirSafe(userEnvironmentPath), pamParseOpts)
	if err != nil {
