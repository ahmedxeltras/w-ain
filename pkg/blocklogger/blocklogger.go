...
var outputChan chan ainshrpc.CommandControllerAppendOutputData

func InitBlockLogger() {
	outputChan = make(chan ainshrpc.CommandControllerAppendOutputData, outputBufferSize)
...
func queueLogData(data ainshrpc.CommandControllerAppendOutputData) {
	select {
...
func writeLogf(blockId string, format string, args []any) {
	...
	data := ainshrpc.CommandControllerAppendOutputData{
		BlockId: blockId,
