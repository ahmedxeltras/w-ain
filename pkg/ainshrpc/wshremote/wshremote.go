...
func (impl *ServerImpl) MessageCommand(ctx context.Context, data ainshrpc.CommandMessageData) error {
	impl.Log("[message] %q\n", data.Message)
	return nil
}

func (impl *ServerImpl) StreamTestCommand(ctx context.Context) chan ainshrpc.RespOrErrorUnion[int] {
	ch := make(chan ainshrpc.RespOrErrorUnion[int], 16)
	go func() {
		defer close(ch)
		idx := 0
		for {
			ch <- ainshrpc.RespOrErrorUnion[int]{Response: idx}
			idx++
			if idx == 1000 {
				break
			}
...
func (impl *ServerImpl) remoteStreamFileDir(ctx context.Context, path string, byteRange ByteRangeType, dataCallback func(fileInfo []*ainshrpc.FileInfo, data []byte, byteRange ByteRangeType)) error {
	innerFilesEntries, err := os.ReadDir(path)
	if err != nil {
		return fmt.Errorf("cannot open dir %q: %w", path, err)
	}
	if byteRange.All {
		if len(innerFilesEntries) > ainshrpc.MaxDirSize {
			innerFilesEntries = innerFilesEntries[:ainshrpc.MaxDirSize]
		}
	} else {
	...
	var fileInfoArr []*ainshrpc.FileInfo
	for _, innerFileEntry := range innerFilesEntries {
		...
		if len(fileInfoArr) >= ainshrpc.DirChunkSize {
...
func (impl *ServerImpl) remoteStreamFileRegular(ctx context.Context, path string, byteRange ByteRangeType, dataCallback func(fileInfo []*ainshrpc.FileInfo, data []byte, byteRange ByteRangeType)) error {
	fd, err := os.Open(path)
	if err != nil {
	...
	buf := make([]byte, ainshrpc.FileChunkSize)
	for {
		if ctx.Err() != nil {
			return ctx.Err()
		}
...
func (impl *ServerImpl) remoteStreamFileInternal(ctx context.Context, data ainshrpc.CommandRemoteStreamFileData, dataCallback func(fileInfo []*ainshrpc.FileInfo, data []byte, byteRange ByteRangeType)) error {
	byteRange, err := parseByteRange(data.ByteRange)
	if err != nil {
		return err
	}
	path, err := ainbase.ExpandHomeDir(data.Path)
	if err != nil {
		return err
	}
	...
	dataCallback([]*ainshrpc.FileInfo{finfo}, nil, byteRange)
	if finfo.NotFound {
		return nil
	}
	if finfo.IsDir {
...
func (impl *ServerImpl) RemoteStreamFileCommand(ctx context.Context, data ainshrpc.CommandRemoteStreamFileData) chan ainshrpc.RespOrErrorUnion[ainshrpc.FileData] {
	ch := make(chan ainshrpc.RespOrErrorUnion[ainshrpc.FileData], 16)
	go func() {
		defer close(ch)
		firstPk := true
		err := impl.remoteStreamFileInternal(ctx, data, func(fileInfo []*ainshrpc.FileInfo, data []byte, byteRange ByteRangeType) {
			resp := ainshrpc.FileData{}
			...
			if len(data) > 0 {
				...
				resp.At = &ainshrpc.FileDataAt{Offset: byteRange.Start, Size: len(data)}
			}
			ch <- ainshrpc.RespOrErrorUnion[ainshrpc.FileData]{Response: resp}
		})
		if err != nil {
...
func (impl *ServerImpl) RemoteTarStreamCommand(ctx context.Context, data ainshrpc.CommandRemoteStreamTarData) <-chan ainshrpc.RespOrErrorUnion[iochantypes.Packet] {
	path := data.Path
	opts := data.Opts
	if opts == nil {
	...
	path, err := ainbase.ExpandHomeDir(path)
	if err != nil {
		return ainshutil.SendErrCh[iochantypes.Packet](fmt.Errorf("cannot expand path %q: %w", path, err))
	}
	cleanedPath := filepath.Clean(ainbase.ExpandHomeDirSafe(path))
	finfo, err := os.Stat(cleanedPath)
	if err != nil {
		return ainshutil.SendErrCh[iochantypes.Packet](fmt.Errorf("cannot stat file %q: %w", path, err))
	}
	...
	go func() {
		...
		if err != nil {
			rtn <- ainshutil.RespErr[iochantypes.Packet](err)
		}
...
func (impl *ServerImpl) RemoteFileCopyCommand(ctx context.Context, data ainshrpc.CommandFileCopyData) (bool, error) {
	...
	destPathCleaned := filepath.Clean(ainbase.ExpandHomeDirSafe(destConn.Path))
	...
	if srcConn.Host == destConn.Host {
		srcPathCleaned := filepath.Clean(ainbase.ExpandHomeDirSafe(srcConn.Path))
		...
		defer timeoutCancel()
		copyStart := time.Now()
		ioch := wshclient.FileStreamTarCommand(wshfs.RpcClient, ainshrpc.CommandRemoteStreamTarData{Path: srcUri, Opts: opts}, &ainshrpc.RpcOpts{Timeout: opts.Timeout})
		numFiles := 0
		numSkipped := 0
		totalBytes := int64(0)
...
func (impl *ServerImpl) RemoteListEntriesCommand(ctx context.Context, data ainshrpc.CommandRemoteListEntriesData) chan ainshrpc.RespOrErrorUnion[ainshrpc.CommandRemoteListEntriesRtnData] {
	...
	go func() {
		...
		if data.Opts.Limit == 0 {
			data.Opts.Limit = ainshrpc.MaxDirSize
		}
		if data.Opts.All {
			...
			innerFilesEntries, err = os.ReadDir(path)
			if err != nil {
				ch <- ainshutil.RespErr[ainshrpc.CommandRemoteListEntriesRtnData](fmt.Errorf("cannot open dir %q: %w", path, err))
				return
			}
		}
		var fileInfoArr []*ainshrpc.FileInfo
		for _, innerFileEntry := range innerFilesEntries {
			if ctx.Err() != nil {
				ch <- ainshutil.RespErr[ainshrpc.CommandRemoteListEntriesRtnData](ctx.Err())
				return
			}
			innerFileInfoInt, err := innerFileEntry.Info()
			if err != nil {
			...
			fileInfoArr = append(fileInfoArr, innerFileInfo)
			if len(fileInfoArr) >= ainshrpc.DirChunkSize {
				resp := ainshrpc.CommandRemoteListEntriesRtnData{FileInfo: fileInfoArr}
				ch <- ainshrpc.RespOrErrorUnion[ainshrpc.CommandRemoteListEntriesRtnData]{Response: resp}
				fileInfoArr = nil
			}
		}
		if len(fileInfoArr) > 0 {
...
func statToFileInfo(fullPath string, finfo fs.FileInfo, extended bool) *ainshrpc.FileInfo {
	mimeType := fileutil.DetectMimeType(fullPath, finfo, extended)
	rtn := &ainshrpc.FileInfo{
		Path:          ainbase.ReplaceHomeDir(fullPath),
...
func computeDirPart(path string) string {
	path = filepath.Clean(ainbase.ExpandHomeDirSafe(path))
...
func (*ServerImpl) fileInfoInternal(path string, extended bool) (*ainshrpc.FileInfo, error) {
	cleanedPath := filepath.Clean(ainbase.ExpandHomeDirSafe(path))
	finfo, err := os.Stat(cleanedPath)
	if os.IsNotExist(err) {
		return &ainshrpc.FileInfo{
			Path:          ainbase.ReplaceHomeDir(path),
...
func resolvePaths(paths []string) string {
	if len(paths) == 0 {
		return ainbase.ExpandHomeDirSafe("~")
	}
	rtnPath := ainbase.ExpandHomeDirSafe(paths[0])
	for _, path := range paths[1:] {
		path = ainbase.ExpandHomeDirSafe(path)
...
func (impl *ServerImpl) RemoteFileJoinCommand(ctx context.Context, paths []string) (*ainshrpc.FileInfo, error) {
	rtnPath := resolvePaths(paths)
	return impl.fileInfoInternal(rtnPath, true)
}

func (impl *ServerImpl) RemoteFileInfoCommand(ctx context.Context, path string) (*ainshrpc.FileInfo, error) {
...
func (impl *ServerImpl) RemoteFileMoveCommand(ctx context.Context, data ainshrpc.CommandFileCopyData) error {
	opts := data.Opts
	destUri := data.DestUri
	srcUri := data.SrcUri
...
func (*ServerImpl) RemoteWriteFileCommand(ctx context.Context, data ainshrpc.FileData) error {
	...
	path, err := ainbase.ExpandHomeDir(data.Info.Path)
	if err != nil {
		return err
	}
...
func (*ServerImpl) RemoteFileDeleteCommand(ctx context.Context, data ainshrpc.CommandDeleteFileData) error {
	expandedPath, err := ainbase.ExpandHomeDir(data.Path)
	if err != nil {
...
func (*ServerImpl) RemoteGetInfoCommand(ctx context.Context) (ainshrpc.RemoteInfo, error) {
	return ainshutil.GetInfo(), nil
}

func (*ServerImpl) RemoteInstallRcFilesCommand(ctx context.Context) error {
	return ainshutil.InstallRcFiles()
}

func (*ServerImpl) FetchSuggestionsCommand(ctx context.Context, data ainshrpc.FetchSuggestionsData) (*ainshrpc.FetchSuggestionsResponse, error) {
