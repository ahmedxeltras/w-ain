...
func parseSimpleId(simpleId string) (discriminator string, value string, err error) {
	...
	if _, err := ainobj.ParseORef(simpleId); err == nil {
...
func resolveThis(ctx context.Context, data ainshrpc.CommandResolveIdsData, value string) (*ainobj.ORef, error) {
	if data.BlockId == "" {
		return nil, fmt.Errorf("no blockid in request")
	}

	if value == SimpleId_This || value == SimpleId_Block {
		return &ainobj.ORef{OType: ainobj.OType_Block, OID: data.BlockId}, nil
	}
	if value == SimpleId_Tab {
		...
		return &ainobj.ORef{OType: ainobj.OType_Tab, OID: tabId}, nil
	}
	if value == SimpleId_Ws || value == SimpleId_Workspace {
		...
		return &ainobj.ORef{OType: ainobj.OType_Workspace, OID: wsId}, nil
	}
	if value == SimpleId_Client || value == SimpleId_Global {
		...
		return &ainobj.ORef{OType: ainobj.OType_Client, OID: client.OID}, nil
	}
	if value == SimpleId_Temp {
		...
		return &ainobj.ORef{OType: "temp", OID: client.TempOID}, nil
	}
	return nil, fmt.Errorf("invalid value for 'this' resolver: %s", value)
}

func resolveORef(_ context.Context, value string) (*ainobj.ORef, error) {
	parsedORef, err := ainobj.ParseORef(value)
	if err != nil {
...
func resolveTabNum(ctx context.Context, data ainshrpc.CommandResolveIdsData, value string) (*ainobj.ORef, error) {
	...
	curTabId, err := ainstore.DBFindTabForBlockId(ctx, data.BlockId)
	if err != nil {
		return nil, fmt.Errorf("error finding tab for block: %v", err)
	}

	wsId, err := ainstore.DBFindWorkspaceForTabId(ctx, curTabId)
	if err != nil {
		return nil, fmt.Errorf("error finding current workspace: %v", err)
	}

	ws, err := ainstore.DBMustGet[*ainobj.Workspace](ctx, wsId)
	if err != nil {
	...
	return &ainobj.ORef{OType: ainobj.OType_Tab, OID: resolvedTabId}, nil
}

func resolveBlock(ctx context.Context, data ainshrpc.CommandResolveIdsData, value string) (*ainobj.ORef, error) {
	...
	layout, err := ainstore.DBGet[*ainobj.LayoutState](ctx, tab.LayoutState)
	if err != nil {
	...
	return &ainobj.ORef{OType: ainobj.OType_Block, OID: leafEntry.BlockId}, nil
}

func resolveView(ctx context.Context, data ainshrpc.CommandResolveIdsData, value string) (*ainobj.ORef, error) {
	...
	layout, err := ainstore.DBMustGet[*ainobj.LayoutState](ctx, tab.LayoutState)
	if err != nil {
	...
	for _, leaf := range *layout.LeafOrder {
		...
		leafBlock, err := ainstore.DBMustGet[*ainobj.Block](ctx, leafBlockId)
		if err != nil {
			continue
		}
		if leafBlock.Meta.GetString("view", "") == viewType {
			...
			if count == instanceNum {
				return &ainobj.ORef{OType: ainobj.OType_Block, OID: leaf.BlockId}, nil
			}
		}
	}
...
func resolveUUID(ctx context.Context, value string) (*ainobj.ORef, error) {
	return ainstore.DBResolveEasyOID(ctx, value)
}

// Main resolver function
func resolveSimpleId(ctx context.Context, data ainshrpc.CommandResolveIdsData, simpleId string) (*ainobj.ORef, error) {
