...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/ainconfig/defaultconfig"
...
type SettingsType struct {
	...
	AinAiShowCloudModes  bool   `json:"ainai:showcloudmodes,omitempty"`
	AinAiDefaultMode     string `json:"ainai:defaultmode,omitempty"`
...
type AIModeConfigType struct {
	...
	AinAICloud         bool     `json:"ainai:cloud,omitempty"`
	AinAIPremium       bool     `json:"ainai:premium,omitempty"`
}
...
type FullConfigType struct {
	...
	Presets        map[string]ainobj.MetaMapType `json:"presets"`
	...
	AinAIModes     map[string]AIModeConfigType   `json:"ainai"`
...
func resolveEnvReplacements(m ainobj.MetaMapType) {
	if m == nil {
		return
	}
	...
	for key, value := range m {
		...
		case map[string]interface{}:
			resolveEnvReplacements(ainobj.MetaMapType(v))
		case []interface{}:
			resolveEnvArray(v)
		}
	}
...
func readConfigHelper(fileName string, barr []byte, readErr error) (ainobj.MetaMapType, []ConfigError) {
...
func readConfigFileFS(fsys fs.FS, logPrefix string, fileName string) (ainobj.MetaMapType, []ConfigError) {
...
func ReadDefaultsConfigFile(fileName string) (ainobj.MetaMapType, []ConfigError) {
	return readConfigFileFS(defaultconfig.ConfigFS, "defaults:", fileName)
}

func ReadWaveHomeConfigFile(fileName string) (ainobj.MetaMapType, []ConfigError) {
	configDirAbsPath := ainbase.GetWaveConfigDir()
	configDirFsys := os.DirFS(configDirAbsPath)
	return readConfigFileFS(configDirFsys, "", fileName)
}

func WriteWaveHomeConfigFile(fileName string, m ainobj.MetaMapType) error {
	configDirAbsPath := ainbase.GetWaveConfigDir()
...
func mergeMetaMapSimple(m ainobj.MetaMapType, toMerge ainobj.MetaMapType) ainobj.MetaMapType {
	if m == nil {
		return toMerge
	}
	if toMerge == nil {
...
func mergeMetaMap(m ainobj.MetaMapType, toMerge ainobj.MetaMapType, simpleMerge bool) ainobj.MetaMapType {
	if simpleMerge {
		return mergeMetaMapSimple(m, toMerge)
	} else {
		return ainobj.MergeMeta(m, toMerge, true)
	}
}
...
func readConfigFilesForDir(fsys fs.FS, logPrefix string, dirName string, fileName string, simpleMerge bool) (ainobj.MetaMapType, []ConfigError) {
...
func readConfigPartForFS(fsys fs.FS, logPrefix string, partName string, simpleMerge bool) (ainobj.MetaMapType, []ConfigError) {
...
func readConfigPart(partName string, simpleMerge bool) (ainobj.MetaMapType, []ConfigError) {
	configDirAbsPath := ainbase.GetWaveConfigDir()
	configDirFsys := os.DirFS(configDirAbsPath)
...
// in golang code, the best way to get the current config is via the watcher -- ainconfig.GetWatcher().GetFullConfig()
func ReadFullConfig() FullConfigType {
	...
	for fieldIdx := 0; fieldIdx < configRType.NumField(); fieldIdx++ {
		...
		var configPart ainobj.MetaMapType
		var errs []ConfigError
		if jsonTag == "-" || jsonTag == "" {
			continue
		} else {
...
func orderConfigKeys(m ainobj.MetaMapType) []string {
...
func jsonMarshalConfigInOrder(m ainobj.MetaMapType) ([]byte, error) {
	if len(m) == 0 {
		return []byte("{}"), nil
	}
...
func SetBaseConfigValue(toMerge ainobj.MetaMapType) error {
	m, cerrs := ReadWaveHomeConfigFile(SettingsFile)
	if len(cerrs) > 0 {
		return fmt.Errorf("error reading config file: %v", cerrs[0])
	}
	if m == nil {
		m = make(ainobj.MetaMapType)
	}
...
func SetConnectionsConfigValue(connName string, toMerge ainobj.MetaMapType) error {
	...
	if connData == nil {
		connData = make(ainobj.MetaMapType)
	}
	for configKey, val := range toMerge {
