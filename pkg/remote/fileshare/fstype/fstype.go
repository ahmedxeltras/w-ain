...
type FileShareClient interface {
	...
	Stat(ctx context.Context, conn *connparse.Connection) (*ainshrpc.FileInfo, error)
	// Read returns the file info at the given path, if it's a directory, then the list of entries
	Read(ctx context.Context, conn *connparse.Connection, data ainshrpc.FileData) (*ainshrpc.FileData, error)
	// ReadStream returns a stream of file data at the given path. If it's a directory, then the list of entries
	ReadStream(ctx context.Context, conn *connparse.Connection, data ainshrpc.FileData) <-chan ainshrpc.RespOrErrorUnion[ainshrpc.FileData]
	// ReadTarStream returns a stream of tar data at the given path
	ReadTarStream(ctx context.Context, conn *connparse.Connection, opts *ainshrpc.FileCopyOpts) <-chan ainshrpc.RespOrErrorUnion[iochantypes.Packet]
	// ListEntries returns the list of entries at the given path, or nothing if the path is a file
	ListEntries(ctx context.Context, conn *connparse.Connection, opts *ainshrpc.FileListOpts) ([]*ainshrpc.FileInfo, error)
	// ListEntriesStream returns a stream of entries at the given path
	ListEntriesStream(ctx context.Context, conn *connparse.Connection, opts *ainshrpc.FileListOpts) <-chan ainshrpc.RespOrErrorUnion[ainshrpc.CommandRemoteListEntriesRtnData]
	// PutFile writes the given data to the file at the given path
	PutFile(ctx context.Context, conn *connparse.Connection, data ainshrpc.FileData) error
	// AppendFile appends the given data to the file at the given path
	AppendFile(ctx context.Context, conn *connparse.Connection, data ainshrpc.FileData) error
	// Mkdir creates a directory at the given path
	Mkdir(ctx context.Context, conn *connparse.Connection) error
	// Move moves the file within the same connection
	MoveInternal(ctx context.Context, srcConn, destConn *connparse.Connection, opts *ainshrpc.FileCopyOpts) error
	// Copy copies the file within the same connection. Returns whether the copy source was a directory
	CopyInternal(ctx context.Context, srcConn, destConn *connparse.Connection, opts *ainshrpc.FileCopyOpts) (bool, error)
	// CopyRemote copies the file between different connections. Returns whether the copy source was a directory
	CopyRemote(ctx context.Context, srcConn, destConn *connparse.Connection, srcClient FileShareClient, opts *ainshrpc.FileCopyOpts) (bool, error)
	...
	Join(ctx context.Context, conn *connparse.Connection, parts ...string) (*ainshrpc.FileInfo, error)
	...
	GetCapability() ainshrpc.FileShareCapability
}
