...
func GetClientPlatform(ctx context.Context, shell genconn.ShellClient) (string, string, error) {
	...
	if err := ainbase.ValidateWshSupportedArch(os, arch); err != nil {
		return "", "", err
	}
	return os, arch, nil
}
...
func CpWshToRemote(ctx context.Context, client *ssh.Client, clientOs string, clientArch string) error {
	...
	wshLocalPath, err := shellutil.GetLocalWshBinaryPath(ainbase.WaveVersion, clientOs, clientArch)
	if err != nil {
		return err
	}
	...
	installWords := map[string]string{
		"installDir":  filepath.ToSlash(filepath.Dir(ainbase.RemoteFullWshBinPath)),
		"tempPath":    ainbase.RemoteFullWshBinPath + ".temp",
		"installPath": ainbase.RemoteFullWshBinPath,
	}
...
func ParseProfiles() []string {
	connfile, cerrs := ainconfig.ReadWaveHomeConfigFile(ainconfig.ProfilesFile)
	if len(cerrs) > 0 {
