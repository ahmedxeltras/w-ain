...
func GetAllConnStatus() []ainshrpc.ConnStatus {
	globalLock.Lock()
	defer globalLock.Unlock()

	var connStatuses []ainshrpc.ConnStatus
...
func (conn *SSHConn) DeriveConnStatus() ainshrpc.ConnStatus {
	conn.Lock.Lock()
	defer conn.Lock.Unlock()
	return ainshrpc.ConnStatus{
		Status:        conn.Status,
...
func (conn *SSHConn) FireConnChangeEvent() {
	...
	event := ainps.WaveEvent{
		Event: ainps.Event_ConnChange,
		Scopes: []string{
...
func (conn *SSHConn) OpenDomainSocketListener(ctx context.Context) error {
	...
	go func() {
		...
		ainshutil.RunWshRpcOverListener(listener)
	}()
	return nil
}
...
func IsWshVersionUpToDate(logCtx context.Context, wshVersionLine string) (bool, string, string, error) {
	...
	expectedVersion := fmt.Sprintf("v%s", ainbase.WaveVersion)
...
func (conn *SSHConn) getWshPath() string {
	...
	return ainbase.RemoteFullWshBinPath
}
...
func (conn *SSHConn) StartConnServer(ctx context.Context, afterUpdate bool) (bool, string, string, error) {
	...
	rpcCtx := ainshrpc.RpcContext{
		RouteId:  ainshutil.MakeConnectionRouteId(conn.GetName()),
		SockName: sockName,
		Conn:     conn.GetName(),
	}
	jwtToken, err := ainshutil.MakeClientJWTToken(rpcCtx)
	if err != nil {
	...
	if isUpToDate && !afterUpdate && os.Getenv(ainbase.WaveWshForceUpdateVarName) != "" {
		isUpToDate = false
		conn.Infof(ctx, "%s set, forcing wsh update\n", ainbase.WaveWshForceUpdateVarName)
	}
	...
	if strings.TrimSpace(jwtLine) == ainbase.NeedJwtConst {
		// write the jwt
	...
	defer cancelFn()
	err = ainshutil.DefaultRouter.WaitForRegister(regCtx, ainshutil.MakeConnectionRouteId(rpcCtx.Conn))
	if err != nil {
...
func (conn *SSHConn) UpdateWsh(ctx context.Context, clientDisplayName string, remoteInfo *ainshrpc.RemoteInfo) error {
...
func (conn *SSHConn) getPermissionToInstallWsh(ctx context.Context, clientDisplayName string) (bool, error) {
	...
	err = ainconfig.SetConnectionsConfigValue(conn.GetName(), meta)
	if err != nil {
	...
	if response.CheckboxStat {
		...
		meta := ainobj.MetaMapType{
			ainconfig.ConfigKey_ConnAskBeforeWshInstall: false,
		}
		setConfigErr := ainconfig.SetBaseConfigValue(meta)
		if setConfigErr != nil {
...
func (conn *SSHConn) Connect(ctx context.Context, connFlags *ainconfig.ConnKeywords) error {
	...
	conn.WithLock(func() {
		if err != nil {
			...
			conn.close_nolock()
			telemetry.GoUpdateActivityWrap(ainshrpc.ActivityUpdate{
...
func (conn *SSHConn) getConnWshSettings() (bool, bool) {
	...
	askBeforeInstall := ainconfig.DefaultBoolPtr(config.Settings.ConnAskBeforeWshInstall, true)
...
func (conn *SSHConn) getConnectionConfig() (ainconfig.ConnKeywords, bool) {
	config := ainconfig.GetWatcher().GetFullConfig()
	connSettings, ok := config.Connections[conn.GetName()]
	if !ok {
		return ainconfig.ConnKeywords{}, false
	}
...
func (conn *SSHConn) connectInternal(ctx context.Context, connFlags *ainconfig.ConnKeywords) error {
...
func EnsureConnection(ctx context.Context, connName string) error {
	...
	case Status_Init, Status_Disconnected:
		return conn.Connect(ctx, &ainconfig.ConnKeywords{})
	case Status_Error:
