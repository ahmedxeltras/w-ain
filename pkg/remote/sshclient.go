...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/trimquotes"
	"github.com/wavetermdev/ainterm/pkg/userinput"
...
func createPublicKeyCallback(connCtx context.Context, sshKeywords *ainconfig.ConnKeywords, authSockSignersExt []ssh.Signer, agentClient agent.ExtendedAgent, debugInfo *ConnectionDebugInfo) func() ([]ssh.Signer, error) {
	...
	for _, identityFile := range sshKeywords.SshIdentityFile {
		filePath, err := ainbase.ExpandHomeDir(identityFile)
		if err != nil {
			continue
		}
...
func createHostKeyCallback(ctx context.Context, sshKeywords *ainconfig.ConnKeywords) (ssh.HostKeyCallback, HostKeyAlgorithms, error) {
	...
	for _, filename := range unexpandedKnownHostsFiles {
		filePath, err := ainbase.ExpandHomeDir(filename)
		if err != nil {
			continue
		}
...
func createClientConfig(connCtx context.Context, sshKeywords *ainconfig.ConnKeywords, debugInfo *ConnectionDebugInfo) (*ssh.ClientConfig, error) {
...
func ConnectToClient(connCtx context.Context, opts *SSHOpts, currentClient *ssh.Client, jumpNum int32, connFlags *ainconfig.ConnKeywords) (*ssh.Client, int32, error) {
	...
	if !ok {
		internalSshConfigKeywords = ainconfig.ConnKeywords{}
	}

	var sshConfigKeywords *ainconfig.ConnKeywords
	...
	parsedKeywords := &ainconfig.ConnKeywords{}
	if opts.SSHUser != "" {
	...
	for _, proxyName := range sshKeywords.SshProxyJump {
		...
		debugInfo.CurrentClient, jumpNum, err = ConnectToClient(connCtx, proxyOpts, debugInfo.CurrentClient, jumpNum, &ainconfig.ConnKeywords{})
		if err != nil {
...
func findSshConfigKeywords(hostPattern string) (connKeywords *ainconfig.ConnKeywords, outErr error) {
	defer func() {
	...
	sshKeywords := &ainconfig.ConnKeywords{}
	var err error
	...
	if identityAgentRaw == "" {
		if runtime.GOOS == "windows" {
			...
			if err == nil {
				...
				if trimmedSock == "" {
					...
					agentPath, err := ainbase.ExpandHomeDir(trimquotes.TryTrimQuotes(trimmedSock))
					if err != nil {
						return nil, err
					}
		...
		agentPath, err := ainbase.ExpandHomeDir(trimquotes.TryTrimQuotes(identityAgentRaw))
		if err != nil {
			return nil, err
		}
		sshKeywords.SshIdentityAgent = utilfn.Ptr(agentPath)
...
func findSshDefaults(hostPattern string) (connKeywords *ainconfig.ConnKeywords, outErr error) {
	sshKeywords := &ainconfig.ConnKeywords{}
...
func mergeKeywords(oldKeywords *ainconfig.ConnKeywords, newKeywords *ainconfig.ConnKeywords) *ainconfig.ConnKeywords {
	if oldKeywords == nil {
		oldKeywords = &ainconfig.ConnKeywords{}
	}
	if newKeywords == nil {
		return oldKeywords
	}
