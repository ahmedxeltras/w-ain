...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/faviconcache"
...
func resolveFileQuery(cwd string, query string) (string, string, string, error) {
	...
	var err error
	cwd, err = ainbase.ExpandHomeDir(cwd)
	if err != nil {
	...
	if query == "~" || strings.HasPrefix(query, tildeSlash) {
		ogQuery := query
		query, err = ainbase.ExpandHomeDir(query)
		if err != nil {
...
func FetchSuggestions(ctx context.Context, data ainshrpc.FetchSuggestionsData) (*ainshrpc.FetchSuggestionsResponse, error) {
...
func filterBookmarksForValid(bookmarks map[string]ainconfig.WebBookmark) map[string]ainconfig.WebBookmark {
	validBookmarks := make(map[string]ainconfig.WebBookmark)
...
func fetchBookmarkSuggestions(_ context.Context, data ainshrpc.FetchSuggestionsData) (*ainshrpc.FetchSuggestionsResponse, error) {
	if data.SuggestionType != "bookmark" {
		return nil, fmt.Errorf("unsupported suggestion type: %q", data.SuggestionType)
	}
	...
	type scoredEntry struct {
		bookmark    ainconfig.WebBookmark
		score       int
	...
	bookmarks := ainconfig.GetWatcher().GetFullConfig().Bookmarks
	...
	var suggestions []ainshrpc.SuggestionType
	for _, entry := range scoredEntries {
		...
		suggestion := ainshrpc.SuggestionType{
			Type:         "url",
	...
	return &ainshrpc.FetchSuggestionsResponse{
...
func fetchFileSuggestions(ctx context.Context, data ainshrpc.FetchSuggestionsData) (*ainshrpc.FetchSuggestionsResponse, error) {
