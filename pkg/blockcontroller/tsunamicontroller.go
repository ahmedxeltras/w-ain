...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/tsunamiutil"
...
func (c *TsunamiController) setManifestMetadata(appId string) {
	manifest, err := ainappstore.ReadAppManifest(appId)
	if err != nil {
		return
	}

	blockRef := ainobj.MakeORef(ainobj.OType_Block, c.blockId)
	...
	ainstore.SetRTInfo(blockRef, rtInfo)
	ainps.Broker.Publish(ainps.WaveEvent{
		Event:  ainps.Event_TsunamiUpdateMeta,
		Scopes: []string{ainobj.MakeORef(ainobj.OType_Block, c.blockId).String()},
...
func (c *TsunamiController) clearSchemas() {
	blockRef := ainobj.MakeORef(ainobj.OType_Block, c.blockId)
	ainstore.SetRTInfo(blockRef, map[string]any{
		"tsunami:schemas": nil,
	})
...
func (c *TsunamiController) Start(ctx context.Context, blockMeta ainobj.MetaMapType, rtOpts *ainobj.RuntimeOpts, force bool) error {
	log.Printf("TsunamiController.Start called for block %s", c.blockId)
	c.runLock.Lock()
	defer c.runLock.Unlock()

	scaffoldPath := ainapputil.GetTsunamiScaffoldPath()
	...
	if sdkVersion == "" {
		sdkVersion = ainapputil.DefaultTsunamiSdkVersion
	}
	goPath := settings.TsunamiGoPath

	appPath := blockMeta.GetString(ainobj.MetaKey_TsunamiAppPath, "")
	appId := blockMeta.GetString(ainobj.MetaKey_TsunamiAppId, "")
	...
	if appPath == "" {
		...
		var err error
		appPath, err = ainappstore.GetAppDir(appId)
		if err != nil {
			return fmt.Errorf("failed to get app directory from tsunami:appid: %w", err)
		}
	} else {
		var err error
		appPath, err = ainbase.ExpandHomeDir(appPath)
		if err != nil {
	...
	if !upToDate || force {
		nodePath := ainbase.GetWaveAppElectronExecPath()
		if nodePath == "" {
...
func runTsunamiAppBinary(ctx context.Context, appBinPath string, appPath string, blockMeta ainobj.MetaMapType) (*TsunamiAppProc, error) {
	...
	tsunamiEnv := blockMeta.GetMap(ainobj.MetaKey_TsunamiEnv)
...
func (c *TsunamiController) sendStatusUpdate() {
	...
	ainps.Broker.Publish(ainps.WaveEvent{
		...
		Scopes: []string{
			ainobj.MakeORef(ainobj.OType_Tab, c.tabId).String(),
			ainobj.MakeORef(ainobj.OType_Block, c.blockId).String(),
		},
		Data: rtStatus,
	})
}
