...
type Controller interface {
	Start(ctx context.Context, blockMeta ainobj.MetaMapType, rtOpts *ainobj.RuntimeOpts, force bool) error
...
func registerController(blockId string, controller Controller) {
	...
	if existingController != nil {
		...
		ainstore.DeleteRTInfo(ainobj.MakeORef(ainobj.OType_Block, blockId))
	}
}
...
func ResyncController(ctx context.Context, tabId string, blockId string, rtOpts *ainobj.RuntimeOpts, force bool) error {
	if tabId == "" || blockId == "" {
	...
	controllerName := blockData.Meta.GetString(ainobj.MetaKey_Controller, "")
	...
	if existing != nil {
		...
		if !needsReplace && (controllerName == BlockController_Shell || controllerName == BlockController_Cmd) {
			connName := blockData.Meta.GetString(ainobj.MetaKey_Connection, "")
	...
	for blockId, controller := range controllers {
		...
		if status != nil && status.ShellProcStatus == Status_Running {
			go func(id string, c Controller) {
				...
				ainstore.DeleteRTInfo(ainobj.MakeORef(ainobj.OType_Block, id))
			}(blockId, controller)
		}
	}
}
...
func getTermSize(bdata *ainobj.Block) ainobj.TermSize {
	if bdata.RuntimeOpts != nil {
		return bdata.RuntimeOpts.TermSize
	} else {
		return ainobj.TermSize{
			Rows: 25,
			Cols: 80,
		}
	}
...
func HandleTruncateBlockFile(blockId string) error {
	ctx, cancelFn := context.WithTimeout(context.Background(), DefaultTimeout)
	defer cancelFn()
	err := filestore.WFS.WriteFile(ctx, blockId, ainbase.BlockFile_Term, nil)
	...
	err = filestore.WFS.DeleteFile(ctx, blockId, ainbase.BlockFile_Cache)
	if err == fs.ErrNotExist {
		err = nil
	}
	if err != nil {
	...
	ainps.Broker.Publish(ainps.WaveEvent{
		Event:  ainps.Event_BlockFile,
		Scopes: []string{ainobj.MakeORef(ainobj.OType_Block, blockId).String()},
		Data: &ainps.WSFileEventData{
			ZoneId:   blockId,
			FileName: ainbase.BlockFile_Term,
			FileOp:   ainps.FileOp_Truncate,
		},
	})
	return nil
...
func CheckConnStatus(blockId string) error {
	bdata, err := ainstore.DBMustGet[*ainobj.Block](context.Background(), blockId)
	if err != nil {
		return fmt.Errorf("error getting block: %w", err)
	}
	connName := bdata.Meta.GetString(ainobj.MetaKey_Connection, "")
	if conncontroller.IsLocalConnName(connName) {
		return nil
	}
