...
func (impl *WaveAppServerImpl) VDomRenderCommand(ctx context.Context, feUpdate vdom.VDomFrontendUpdate) chan ainshrpc.RespOrErrorUnion[*vdom.VDomBackendUpdate] {
	respChan := make(chan ainshrpc.RespOrErrorUnion[*vdom.VDomBackendUpdate], 5)
	defer func() {
		panicErr := panichandler.PanicHandler("VDomRenderCommand", recover())
		if panicErr != nil {
			respChan <- ainshrpc.RespOrErrorUnion[*vdom.VDomBackendUpdate]{
				Error: panicErr,
			}
			close(respChan)
		}
...
func (impl *WaveAppServerImpl) VDomUrlRequestCommand(ctx context.Context, data ainshrpc.VDomUrlRequestData) chan ainshrpc.RespOrErrorUnion[ainshrpc.VDomUrlRequestResponse] {
	respChan := make(chan ainshrpc.RespOrErrorUnion[ainshrpc.VDomUrlRequestResponse])
