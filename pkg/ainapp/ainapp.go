...
func (client *Client) Connect() error {
	...
	if jwtToken == "" {
		return fmt.Errorf("no %s env var set", ainshutil.WaveJwtTokenVarName)
	}
	...
	rpcClient, err := ainshutil.SetupDomainSocketRpcClient(sockName, client.ServerImpl, "vdomclient")
	if err != nil {
		return fmt.Errorf("error setting up domain socket rpc client: %v", err)
	}
	client.RpcClient = rpcClient
	_, err = wshclient.AuthenticateCommand(client.RpcClient, jwtToken, &ainshrpc.RpcOpts{Route: ainshutil.ControlRoute})
	if err != nil {
...
func (c *Client) CreateVDomContext(target *vdom.VDomTarget) error {
	blockORef, err := wshclient.VDomCreateContextCommand(
		...
		&ainshrpc.RpcOpts{Route: ainshutil.MakeFeBlockRouteId(c.RpcContext.BlockId)},
	)
	if err != nil {
		return err
	}
	...
	gotRoute, err := wshclient.WaitForRouteCommand(c.RpcClient, ainshrpc.CommandWaitForRouteData{
		RouteId: ainshutil.MakeFeBlockRouteId(blockORef.OID),
		WaitMs:  4000,
	}, &ainshrpc.RpcOpts{Timeout: 5000})
	if err != nil {
	...
	wshclient.EventSubCommand(c.RpcClient, ainps.SubscriptionRequest{Event: ainps.Event_BlockClose, Scopes: []string{
		blockORef.String(),
	}}, nil)
	c.RpcClient.EventListener.On("blockclose", func(event *ainps.WaveEvent) {
...
func (c *Client) SendAsyncInitiation() error {
	...
	return wshclient.VDomAsyncInitiationCommand(
		...
		&ainshrpc.RpcOpts{Route: ainshutil.MakeFeBlockRouteId(c.VDomContextBlockId)},
	)
}
...
func determineMimeType(option FileHandlerOption) (string, []byte) {
	...
	if option.FilePath != "" {
		filePath := ainbase.ExpandHomeDirSafe(option.FilePath)
