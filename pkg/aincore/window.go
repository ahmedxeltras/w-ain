...
func SwitchWorkspace(ctx context.Context, windowId string, workspaceId string) (*ainobj.Workspace, error) {
	...
	allWindows, err := ainstore.DBGetAllObjsByType[*ainobj.Window](ctx, ainobj.OType_Window)
	if err != nil {
	...
	for _, w := range allWindows {
		if w.WorkspaceId == workspaceId {
			...
			err = wshclient.FocusWindowCommand(client, w.OID, &ainshrpc.RpcOpts{Route: ainshutil.ElectronRoute})
			return nil, err
		}
	}
	window.WorkspaceId = workspaceId
	err = ainstore.DBUpdate(ctx, window)
	if err != nil {
...
func GetWindow(ctx context.Context, windowId string) (*ainobj.Window, error) {
...
func CreateWindow(ctx context.Context, winSize *ainobj.WinSize, workspaceId string) (*ainobj.Window, error) {
	log.Printf("CreateWindow %v %v\n", winSize, workspaceId)
	var ws *ainobj.Workspace
	if workspaceId == "" {
	...
	if winSize == nil {
		winSize = &ainobj.WinSize{
			Width:  0,
			Height: 0,
		}
	}
	window := &ainobj.Window{
		OID:         windowId,
		WorkspaceId: ws.OID,
		IsNew:       true,
		Pos: ainobj.Point{
	...
	err := ainstore.DBInsert(ctx, window)
	if err != nil {
...
func CloseWindow(ctx context.Context, windowId string, fromElectron bool) error {
	...
	if err == nil {
		...
		err = ainstore.DBDelete(ctx, ainobj.OType_Window, windowId)
		if err != nil {
...
func CheckAndFixWindow(ctx context.Context, windowId string) *ainobj.Window {
...
func FocusWindow(ctx context.Context, windowId string) error {
	...
	return ainstore.DBUpdate(ctx, client)
}
