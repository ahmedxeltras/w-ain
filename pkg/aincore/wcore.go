...
func EnsureInitialData() (bool, error) {
	...
	if client.TempOID == "" {
		...
		err = ainstore.DBUpdate(ctx, client)
		if err != nil {
...
func CreateClient(ctx context.Context) (*ainobj.Client, error) {
	client := &ainobj.Client{
		OID:       uuid.NewString(),
		WindowIds: []string{},
	}
	err := ainstore.DBInsert(ctx, client)
	if err != nil {
...
func GetClientData(ctx context.Context) (*ainobj.Client, error) {
...
func SendWaveObjUpdate(oref ainobj.ORef) {
	ctx, cancelFn := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancelFn()
	// send a waveobj:update event
	waveObj, err := ainstore.DBGetORef(ctx, oref)
	if err != nil {
		log.Printf("error getting object for update event: %v", err)
		return
	}
	ainps.Broker.Publish(ainps.WaveEvent{
		Event:  ainps.Event_WaveObjUpdate,
		Scopes: []string{oref.String()},
		Data: ainobj.WaveObjUpdate{
			UpdateType: ainobj.UpdateType_Update,
			OType:      waveObj.GetOType(),
			OID:        ainobj.GetOID(waveObj),
			Obj:        waveObj,
		},
	})
}
...
func GoSendNoTelemetryUpdate(telemetryEnabled bool) {
	go func() {
		...
		err = aincloud.SendNoTelemetryUpdate(ctx, clientData.OID, !telemetryEnabled)
		if err != nil {
...
func InitMainServer() error {
	...
	mainServer, err := ainstore.DBGetSingleton[*ainobj.MainServer](ctx)
	if err == ainstore.ErrNotFound {
		mainServer = &ainobj.MainServer{
			OID: uuid.NewString(),
		}
		err = ainstore.DBInsert(ctx, mainServer)
		if err != nil {
	...
	if mainServer.JwtPrivateKey == "" || mainServer.JwtPublicKey == "" {
		keyPair, err := ainjwt.GenerateKeyPair()
		if err != nil {
	...
	if needsUpdate {
		err = ainstore.DBUpdate(ctx, mainServer)
		if err != nil {
	...
	err = ainjwt.SetPrivateKey(privateKeyBytes)
	if err != nil {
		return fmt.Errorf("error setting jwt private key: %w", err)
	}
	err = ainjwt.SetPublicKey(publicKeyBytes)
	if err != nil {
