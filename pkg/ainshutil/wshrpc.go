...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/util/ds"
...
type WshRpc struct {
	...
	RpcContext         *atomic.Pointer[ainshrpc.RpcContext]
...
func MakeWshRpcWithChannels(inputCh chan baseds.RpcInputChType, outputCh chan []byte, rpcCtx ainshrpc.RpcContext, serverImpl ServerImpl, debugName string) *WshRpc {
	if inputCh == nil {
...
func MakeWshRpc(rpcCtx ainshrpc.RpcContext, serverImpl ServerImpl, debugName string) *WshRpc {
	return MakeWshRpcWithChannels(nil, nil, rpcCtx, serverImpl, debugName)
}

func (w *WshRpc) GetRpcContext() ainshrpc.RpcContext {
	rtnPtr := w.RpcContext.Load()
	return *rtnPtr
}

func (w *WshRpc) SetRpcContext(ctx ainshrpc.RpcContext) {
...
func (w *WshRpc) handleRequestInternal(req *RpcMessage, ingressLinkId baseds.LinkId, pprofCtx context.Context) {
	if req.Command == ainshrpc.Command_EventRecv {
...
func (w *WshRpc) runServer() {
	defer func() {
		panichandler.PanicHandler("ainshrpc.runServer", recover())
		close(w.OutputCh)
		w.setServerDone()
	}()
outer:
...
// no response
func (w *WshRpc) SendCommand(command string, data any, opts *ainshrpc.RpcOpts) error {
	var optsCopy ainshrpc.RpcOpts
	if opts != nil {
		optsCopy = *opts
	}
...
// single response
func (w *WshRpc) SendRpcRequest(command string, data any, opts *ainshrpc.RpcOpts) (any, error) {
	var optsCopy ainshrpc.RpcOpts
	if opts != nil {
		optsCopy = *opts
	}
...
func (handler *RpcResponseHandler) GetRpcContext() ainshrpc.RpcContext {
...
func (handler *RpcResponseHandler) SendMessage(msg string) {
	rpcMsg := &RpcMessage{
		Command: ainshrpc.Command_Message,
		Data: ainshrpc.CommandMessageData{
			Message: msg,
		},
...
func (w *WshRpc) SendComplexRequest(command string, data any, opts *ainshrpc.RpcOpts) (rtnHandler *RpcRequestHandler, rtnErr error) {
	if w.IsServerDone() {
...
func (w *WshRpc) setServerDone() {
	...
	w.ServerDone = true
	close(w.CtxDoneCh)
	utilfn.DrainChannelSafe(w.InputCh, "ainshrpc.setServerDone")
}
