...
const WaveJwtTokenVarName = ainbase.WaveJwtTokenVarName
...
func SetupPacketRpcClient(input io.Reader, output io.Writer, serverImpl ServerImpl, debugStr string) (*WshRpc, chan []byte) {
	...
	rpcClient := MakeWshRpcWithChannels(messageCh, outputCh, ainshrpc.RpcContext{}, serverImpl, debugStr)
...
func SetupConnRpcClient(conn net.Conn, serverImpl ServerImpl, debugStr string) (*WshRpc, chan error, error) {
	...
	rtn := MakeWshRpcWithChannels(inputCh, outputCh, ainshrpc.RpcContext{}, serverImpl, debugStr)
...
func SetupDomainSocketRpcClient(sockName string, serverImpl ServerImpl, debugName string) (*WshRpc, error) {
	sockName = ainbase.ExpandHomeDirSafe(sockName)
...
func MakeClientJWTToken(rpcCtx ainshrpc.RpcContext) (string, error) {
	...
	return ainjwt.Sign(claims)
}

func claimsToRpcCtx(claims *ainjwt.WaveJwtClaims) *ainshrpc.RpcContext {
	return &ainshrpc.RpcContext{
...
func ValidateAndExtractRpcContextFromToken(tokenStr string) (*ainshrpc.RpcContext, error) {
	claims, err := ainjwt.ValidateAndExtract(tokenStr)
	if err != nil {
		return nil, err
	}
...
func ExtractUnverifiedRpcContext(tokenStr string) (*ainshrpc.RpcContext, error) {
	token, _, err := new(jwt.Parser).ParseUnverified(tokenStr, &ainjwt.WaveJwtClaims{})
	if err != nil {
		return nil, fmt.Errorf("error parsing token: %w", err)
	}
	claims, ok := token.Claims.(*ainjwt.WaveJwtClaims)
	if !ok {
...
func GetInfo() ainshrpc.RemoteInfo {
	return ainshrpc.RemoteInfo{
		ClientArch:    runtime.GOARCH,
		ClientOs:      runtime.GOOS,
		ClientVersion: ainbase.WaveVersion,
		Shell:         getShell(),
	}
...
func InstallRcFiles() error {
	...
	waveDir := filepath.Join(home, ainbase.RemoteWaveHomeDirName)
	wshBinDir := filepath.Join(waveDir, ainbase.RemoteWshBinDirName)
	return shellutil.InitRcFiles(waveDir, wshBinDir)
}

func SendErrCh[T any](err error) <-chan ainshrpc.RespOrErrorUnion[T] {
	ch := make(chan ainshrpc.RespOrErrorUnion[T], 1)
	ch <- RespErr[T](err)
	close(ch)
	return ch
}

func RespErr[T any](err error) ainshrpc.RespOrErrorUnion[T] {
	return ainshrpc.RespOrErrorUnion[T]{Error: err}
}
