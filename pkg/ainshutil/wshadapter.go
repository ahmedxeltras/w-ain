...
var WshCommandDeclMap = ainshrpc.GenerateWshCommandDeclMap()
...
func serverImplAdapter(impl any) func(*RpcResponseHandler) bool {
	...
	return func(handler *RpcResponseHandler) bool {
		...
		if rmethod == nil {
			if !handler.NeedsResponse() && cmd != ainshrpc.Command_Message {
		...
		if methodDecl.CommandType == ainshrpc.RpcType_Call {
			...
			return true
		} else if methodDecl.CommandType == ainshrpc.RpcType_ResponseStream {
			rtnVals := implMethod.Call(callParams)
			rtnChVal := rtnVals[0]
			if rtnChVal.IsNil() {
