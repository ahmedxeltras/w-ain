...
func (router *WshRouter) SendEvent(routeId string, event ainps.WaveEvent) {
	defer func() {
	...
	msg := RpcMessage{
		Command: ainshrpc.Command_EventRecv,
		Route:   routeId,
		Data:    event,
	}
...
func (router *WshRouter) handleNoRoute(msg RpcMessage, ingressLinkId baseds.LinkId) {
	...
	if msg.ReqId == "" {
		if msg.Command == ainshrpc.Command_Message {
		...
		respMsg := RpcMessage{
			...
			Route:   msg.Source,
			Source:  ControlRoute,
			Data:    ainshrpc.CommandMessageData{Message: nrErr.Error()},
		}
...
func (router *WshRouter) registerControlPlane() {
	...
	controlRpcCtx := ainshrpc.RpcContext{RouteId: ControlRoute}
...
func (router *WshRouter) announceUpstream(routeId string) {
	msg := RpcMessage{
		Command: ainshrpc.Command_RouteAnnounce,
		Route:   ControlRoute,
		Source:  routeId,
	}
...
func (router *WshRouter) unannounceUpstream(routeId string) {
	msg := RpcMessage{
		Command: ainshrpc.Command_RouteUnannounce,
		Route:   ControlRoute,
		Source:  routeId,
	}
	msgBytes, _ := json.Marshal(msg)
...
func (router *WshRouter) unsubscribeFromBroker(routeId string) {
	...
	ainps.Broker.UnsubscribeAll(routeId)
	ainps.Broker.Publish(ainps.WaveEvent{Event: ainps.Event_RouteGone, Scopes: []string{routeId}})
}
