...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/util/daystr"
...
type TelemetryData struct {
	...
	Displays            []ainshrpc.ActivityDisplayType `json:"displays,omitempty"`
...
func GoUpdateActivityWrap(update ainshrpc.ActivityUpdate, debugStr string) {
	go func() {
		defer func() {
...
func insertTEvent(ctx context.Context, event *telemetrydata.TEvent) error {
	...
	return ainstore.WithTx(ctx, func(tx *ainstore.TxWrap) error {
...
func GetNonUploadedTEvents(ctx context.Context, maxEvents int) ([]*telemetrydata.TEvent, error) {
	now := time.Now()
	return ainstore.WithTxRtn(ctx, func(tx *ainstore.TxWrap) ([]*telemetrydata.TEvent, error) {
...
func UpdateActivity(ctx context.Context, update ainshrpc.ActivityUpdate) error {
	now := time.Now()
	dayStr := daystr.GetCurDayStr()
	txErr := ainstore.WithTx(ctx, func(tx *ainstore.TxWrap) error {
		...
		if !found {
			...
			tx.Exec(query, dayStr, tdata, tzName, tzOffset, ainbase.WaveVersion, ainbase.ClientArch(), ainbase.BuildTime, ainbase.UnameKernelRelease())
		}
		...
		query = `UPDATE db_activity
                 SET tdata = ?,
                     ...
                     buildtime = ?
                 WHERE day = ?`
		tx.Exec(query, tdata, ainbase.WaveVersion, ainbase.BuildTime, dayStr)
		return nil
	})
	if txErr != nil {
		return txErr
