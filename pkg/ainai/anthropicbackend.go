...
func (AnthropicBackend) StreamCompletion(ctx context.Context, request ainshrpc.WaveAIStreamRequest) chan ainshrpc.RespOrErrorUnion[ainshrpc.WaveAIPacketType] {
	...
	go func() {
		...
		for {
			...
			case "message_delta":
				...
				if event.Usage != nil {
					pk := MakeWaveAIPacket()
					pk.Usage = &ainshrpc.WaveAIUsageType{
