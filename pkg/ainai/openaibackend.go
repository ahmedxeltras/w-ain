...
func setApiType(opts *ainshrpc.WaveAIOptsType, clientConfig *openaiapi.ClientConfig) error {
...
func convertPrompt(prompt []ainshrpc.WaveAIPromptMessageType) []openaiapi.ChatCompletionMessage {
...
func (OpenAIBackend) StreamCompletion(ctx context.Context, request ainshrpc.WaveAIStreamRequest) chan ainshrpc.RespOrErrorUnion[ainshrpc.WaveAIPacketType] {
	...
	go func() {
		...
		for {
			...
			if streamResp.Model != "" && !sentHeader {
				...
				rtn <- ainshrpc.RespOrErrorUnion[ainshrpc.WaveAIPacketType]{Response: *pk}
				sentHeader = true
			}
