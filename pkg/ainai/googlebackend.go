...
func (GoogleBackend) StreamCompletion(ctx context.Context, request ainshrpc.WaveAIStreamRequest) chan ainshrpc.RespOrErrorUnion[ainshrpc.WaveAIPacketType] {
	...
	go func() {
		...
		for {
			...
			rtn <- ainshrpc.RespOrErrorUnion[ainshrpc.WaveAIPacketType]{Response: ainshrpc.WaveAIPacketType{Text: convertCandidatesToText(resp.Candidates)}}
		}
	}()
	return rtn
}

func extractHistory(history []ainshrpc.WaveAIPromptMessageType) []*genai.Content {
...
func extractPrompt(prompt []ainshrpc.WaveAIPromptMessageType) genai.Part {
