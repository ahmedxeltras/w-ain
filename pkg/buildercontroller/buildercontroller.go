...
func GetBuilderAppExecutablePath(appPath string) (string, error) {
	...
	err := ainbase.TryMkdirs(binDir, 0755, "app bin directory")
	if err != nil {
...
func (bc *BuilderController) buildAndRun(ctx context.Context, appId string, builderEnv map[string]string, resultCh chan<- *BuildResult) {
	appNS, _, err := ainappstore.ParseAppId(appId)
	if err != nil {
...
func (bc *BuilderController) runBuilderApp(ctx context.Context, appId string, appBinPath string, builderEnv map[string]string) (*BuilderProcess, error) {
	...
	secretBindings, err := ainappstore.ReadAppSecretBindings(appId)
	if err != nil {
		return nil, fmt.Errorf("failed to read secret bindings (ERR-SECRET): %w", err)
	}

	secretEnv, err := ainappstore.BuildAppSecretEnv(appId, manifest, secretBindings)
	if err != nil {
...
func (bc *BuilderController) GetStatus() ainshrpc.BuilderStatusData {
	bc.statusLock.Lock()
	defer bc.statusLock.Unlock()

	bc.statusVersion++
	statusData := ainshrpc.BuilderStatusData{
		Status:   bc.status,
		Port:     bc.port,
	...
	if bc.appId != "" {
		manifest, err := ainappstore.ReadAppManifest(bc.appId)
		if err == nil && manifest != nil {
			wshrpcManifest := &ainshrpc.AppManifest{
				AppMeta: ainshrpc.AppMeta{
					Title:     manifest.AppMeta.Title,
					ShortDesc: manifest.AppMeta.ShortDesc,
				},
				ConfigSchema: manifest.ConfigSchema,
				DataSchema:   manifest.DataSchema,
				Secrets:      make(map[string]ainshrpc.SecretMeta),
			}
			for k, v := range manifest.Secrets {
				wshrpcManifest.Secrets[k] = ainshrpc.SecretMeta{
					Desc:     v.Desc,
					Optional: v.Optional,
				}
			}
		...
		secretBindings, err := ainappstore.ReadAppSecretBindings(bc.appId)
		if err == nil {
			statusData.SecretBindings = secretBindings
		}

		if manifest != nil && secretBindings != nil {
			_, err := ainappstore.BuildAppSecretEnv(bc.appId, manifest, secretBindings)
...
func (bc *BuilderController) publishStatus() {
	...
	ainps.Broker.Publish(ainps.WaveEvent{
		Event:  ainps.Event_BuilderStatus,
		Scopes: []string{ainobj.MakeORef(ainobj.OType_Builder, bc.builderId).String()},
		Data:   status,
	})
}
...
func (bc *BuilderController) publishOutputLine(line string, reset bool) {
	ainps.Broker.Publish(ainps.WaveEvent{
		Event:  ainps.Event_BuilderOutput,
		Scopes: []string{ainobj.MakeORef(ainobj.OType_Builder, bc.builderId).String()},
		Data: map[string]any{
			"lines": []string{line},
			"reset": reset,
