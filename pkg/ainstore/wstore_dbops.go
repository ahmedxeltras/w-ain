...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/util/dbutil"
)

var ErrNotFound = fmt.Errorf("not found")

func waveObjTableName(w ainobj.WaveObj) string {
...
func tableNameGen[T ainobj.WaveObj]() string {
	var zeroObj T
	return tableNameFromOType(zeroObj.GetOType())
}

func getOTypeGen[T ainobj.WaveObj]() string {
	var zeroObj T
	return zeroObj.GetOType()
}

func DBGetCount[T ainobj.WaveObj](ctx context.Context) (int, error) {
...
func DBGetSingleton[T ainobj.WaveObj](ctx context.Context) (T, error) {
	rtn, err := DBGetSingletonByType(ctx, getOTypeGen[T]())
	return genericCastWithErr[T](rtn, err)
}

func DBGetSingletonByType(ctx context.Context, otype string) (ainobj.WaveObj, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (ainobj.WaveObj, error) {
		...
		rtn, err := ainobj.FromJson(row.Data)
		if err != nil {
			return rtn, err
		}
		ainobj.SetVersion(rtn, row.Version)
		return rtn, nil
	})
}

func DBExistsORef(ctx context.Context, oref ainobj.ORef) (bool, error) {
...
func DBGet[T ainobj.WaveObj](ctx context.Context, id string) (T, error) {
	rtn, err := DBGetORef(ctx, ainobj.ORef{OType: getOTypeGen[T](), OID: id})
	return genericCastWithErr[T](rtn, err)
}

func DBMustGet[T ainobj.WaveObj](ctx context.Context, id string) (T, error) {
	rtn, err := DBGetORef(ctx, ainobj.ORef{OType: getOTypeGen[T](), OID: id})
	if err != nil {
		var zeroVal T
		return zeroVal, err
...
func DBGetORef(ctx context.Context, oref ainobj.ORef) (ainobj.WaveObj, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (ainobj.WaveObj, error) {
		table := tableNameFromOType(oref.OType)
...
func dbSelectOIDs(ctx context.Context, otype string, oids []string) ([]ainobj.WaveObj, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) ([]ainobj.WaveObj, error) {
		table := tableNameFromOType(otype)
		...
		rtn := make([]ainobj.WaveObj, 0, len(rows))
		for _, row := range rows {
			waveObj, err := ainobj.FromJson(row.Data)
			if err != nil {
				return nil, err
			}
			ainobj.SetVersion(waveObj, row.Version)
...
func DBGetAllObjsByType[T ainobj.WaveObj](ctx context.Context, otype string) ([]T, error) {
...
func DBResolveEasyOID(ctx context.Context, oid string) (*ainobj.ORef, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (*ainobj.ORef, error) {
		for _, rtype := range ainobj.AllWaveObjTypes() {
			otype := reflect.Zero(rtype).Interface().(ainobj.WaveObj).GetOType()
			table := tableNameFromOType(otype)
			var fullOID string
			if len(oid) == 8 {
			...
			if fullOID != "" {
				oref := ainobj.MakeORef(otype, fullOID)
				return &oref, nil
			}
		}
		return nil, ErrNotFound
...
func DBSelectMap[T ainobj.WaveObj](ctx context.Context, ids []string) (map[string]T, error) {
	...
	for _, obj := range rtnArr {
		rtnMap[ainobj.GetOID(obj)] = obj.(T)
	}
	return rtnMap, nil
}
...
func DBDelete(ctx context.Context, otype string, id string) error {
	err := WithTx(ctx, func(tx *TxWrap) error {
		...
		tx.Exec(query, id)
		ainobj.ContextAddUpdate(ctx, ainobj.WaveObjUpdate{UpdateType: ainobj.UpdateType_Delete, OType: otype, OID: id})
		return nil
	})
	if err != nil {
		return err
...
func DBUpdate(ctx context.Context, val ainobj.WaveObj) error {
	oid := ainobj.GetOID(val)
	if oid == "" {
		return fmt.Errorf("cannot update %T value with empty id", val)
	}
	jsonData, err := ainobj.ToJson(val)
	if err != nil {
		return err
	}
	return WithTx(ctx, func(tx *TxWrap) error {
		...
		ainobj.SetVersion(val, newVersion)
		ainobj.ContextAddUpdate(ctx, ainobj.WaveObjUpdate{UpdateType: ainobj.UpdateType_Update, OType: val.GetOType(), OID: oid, Obj: val})
		return nil
	})
}

func DBInsert(ctx context.Context, val ainobj.WaveObj) error {
	oid := ainobj.GetOID(val)
	if oid == "" {
...
func DBFindTabForBlockId(ctx context.Context, blockId string) (string, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (string, error) {
		...
		for {
			...
			oref, err := ainobj.ParseORef(parentORef)
			if err != nil {
