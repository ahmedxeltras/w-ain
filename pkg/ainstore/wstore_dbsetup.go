...
import (
	...
	"github.com/wavetermdev/ainterm/pkg/util/migrateutil"

	dbfs "github.com/wavetermdev/ainterm/db"
)
...
func GetDBName() string {
	waveHome := ainbase.GetWaveDataDir()
	return filepath.Join(waveHome, ainbase.WaveDBDir, WStoreDBName)
}
...
func WithTx(ctx context.Context, fn func(tx *TxWrap) error) (rtnErr error) {
	ainobj.ContextUpdatesBeginTx(ctx)
	defer func() {
		if rtnErr != nil {
			ainobj.ContextUpdatesRollbackTx(ctx)
		} else {
			ainobj.ContextUpdatesCommitTx(ctx)
		}
	}()
