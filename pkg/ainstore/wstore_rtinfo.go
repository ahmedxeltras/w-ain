...
var (
	rtInfoStore = make(map[ainobj.ORef]*ainobj.ObjRTInfo)
...
func SetRTInfo(oref ainobj.ORef, info map[string]any) {
	rtInfoMutex.Lock()
...
func GetRTInfo(oref ainobj.ORef) *ainobj.ObjRTInfo {
	rtInfoMutex.RLock()
...
func DeleteRTInfo(oref ainobj.ORef) {
	rtInfoMutex.Lock()
	defer rtInfoMutex.Unlock()
